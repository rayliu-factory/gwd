---
name: gwd-orchestrator
description: >
  Build software products autonomously via GWD headless mode. Handles the full
  ...
  "create a project", "run gwd", "check build status", or any task that
...
<objective>
You are an autonomous agent that builds software by orchestrating GWD as a subprocess.
GWD is a headless CLI that plans, codes, tests, and ships software from a spec.
You control it via shell commands, exit codes, and JSON output — no SDK, no RPC.
</objective>

<mental_model>
GWD headless is a subprocess you launch and monitor. Think of it like a junior developer
you hand a spec to:

1. You write the spec (what to build)
2. You launch the build (`gwd headless ... new-milestone --context spec.md --auto`)
...
You never write application code yourself — GWD does that.
</mental_model>

<critical_rules>
- **Flags before command.** `gwd headless [--flags] [command] [args]`. Flags after the command are ignored.
...
- **One project directory per build.** Each GWD project needs its own directory with a `.gwd/` folder.
</critical_rules>
...
Build a ...
EOF
gwd headless --output-format json --context spec.md new-milestone --auto 2>/dev/null
```

**Check project state (instant, free):**
```bash
cd /path/to/project
gwd headless query | jq '{phase: .state.phase, progress: .state.progress, cost: .cost.total}'
```
...
| `1`  | Error or timeout | Inspect stderr, check `.gwd/STATE.md`, retry or escalate |
...
<project_structure>
GWD creates and manages all state in `.gwd/`:
```
.gwd/
...
State is derived from files on disk — checkboxes in ROADMAP.md and PLAN.md are the source of truth for completion. You never need to edit these files. GWD manages them. But you can read them to understand progress.
</project_structure>

...
When using `--output-format json`, GWD collects events silently and emits a single `HeadlessJsonResult` JSON object to stdout at process exit. This is the structured result for orchestrator decision-making.
...
| `phase` | `string \| undefined` | Current GWD phase at session end (e.g. `"executing"`, `"blocked"`, `"complete"`). |
...
gwd headless --resume "$SESSION_ID" --output-format json next 2>/dev/null
```

### Artifact Collection

```bash
RESULT=$(gwd headless --output-format json auto 2>/dev/null)
...
```json
{
  ...
  "nextAction": "dispatch",
  "artifacts": [
    ".gwd/milestones/M001/slices/S01/tasks/T01-SUMMARY.md"
  ],
  "commits": [
    "a1b2c3d"
  ]
...
gwd headless query | jq '{phase: .state.phase, progress: .state.progress, totalCost: .cost.total}'
```

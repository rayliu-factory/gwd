...
- **SCM provider** — "GWD Agent" appears in Source Control panel with accept/discard per-file diffs
...
- **Slash completion** filtered to `/gwd` commands only
- **Checkpoint labels** show timestamp + first action (e.g., "10:32 — Edit sidebar.ts")
- **Session tree** supports ISO timestamp filenames (GWD's actual format)
...
- **Sidebar reduced** from 6 panels to 3 (GWD Agent, Sessions, Activity)
...
- **Enhanced code lens** — Refactor, Find Bugs, and Generate Tests alongside Ask GWD
...
- Full RPC client — spawns `gwd --mode rpc`, JSON line framing, all RPC commands
- Sidebar dashboard — connection status, model info, thinking level, token usage, cost, quick actions
- Chat participant — `@gwd` in VS Code Chat with streaming responses
...
- Slash command completion — auto-complete for `/gwd` commands
- Code lens — "Ask GWD" above functions and classes in TS/JS/Python/Go/Rust

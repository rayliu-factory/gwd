...
describe('checkExistingEnvKeys', () => {
  ...
  it('finds key in process.env', async () => {
    const tmp = makeTempDir('env-check');
    const saved = process.env.GWD_MCP_TEST_KEY_1;
    try {
      process.env.GWD_MCP_TEST_KEY_1 = 'some-value';
      const envPath = join(tmp, '.env');
      const result = await checkExistingEnvKeys(['GWD_MCP_TEST_KEY_1'], envPath);
      assert.deepStrictEqual(result, ['GWD_MCP_TEST_KEY_1']);
    } finally {
      delete process.env.GWD_MCP_TEST_KEY_1;
      if (saved !== undefined) process.env.GWD_MCP_TEST_KEY_1 = saved;
      rmSync(tmp, { recursive: true, force: true });
    }
  });
...
describe('applySecrets', () => {
  ...
  it('writes keys to .env and hydrates process.env', async () => {
    ...
    const envPath = join(tmp, '.env');
    savedKeys.GWD_APPLY_TEST_A = process.env.GWD_APPLY_TEST_A;
    try {
      const { applied, errors } = await applySecrets(
        [{ key: 'GWD_APPLY_TEST_A', value: 'val-a' }],
        'dotenv',
        { envFilePath: envPath },
      );
      assert.deepStrictEqual(applied, ['GWD_APPLY_TEST_A']);
      assert.deepStrictEqual(errors, []);
      assert.equal(process.env.GWD_APPLY_TEST_A, 'val-a');
      const content = readFileSync(envPath, 'utf8');
      assert.ok(content.includes('GWD_APPLY_TEST_A=val-a'));
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  });

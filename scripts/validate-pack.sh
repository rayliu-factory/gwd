...
# --- Guard: workspace packages must not have @gwd/* cross-deps ---
echo "==> Checking workspace packages for @gwd/* cross-deps..."
CROSS_FAILED=0
...
if [ "$CROSS_FAILED" = "1" ]; then
  echo "ERROR: Workspace packages have @gwd/* cross-dependencies."
  echo "    These cause 404s when npm resolves them from the registry."
  exit 1
fi
echo "    No @gwd/* cross-dependencies."

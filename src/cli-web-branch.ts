...
export interface CliFlags {
  ...
  messages: string[]
  web?: boolean
  /** Optional project path for web mode: `gwd --web <path>` or `gwd web start <path>` */
  webPath?: string
...
function emitWebModeFailure(stderr: WritableLike, status: WebModeLaunchStatus): void {
  if (status.ok) return
  stderr.write(`[gwd] Web mode launch failed: ${status.failureReason}\n`)
}
...
export async function runWebCliBranch(
  ...
  // Handle `gwd web stop [path|--all]` subcommand
  ...
  // `gwd web [start] [path]` is an alias for `gwd --web [path]`
  // Matches: `gwd web`, `gwd web start`, `gwd web start <path>`, `gwd web <path>`
  ...
  //   gwd --web <path>           → flags.webPath
  //   gwd web start <path>       → messages[2]
  //   gwd web <path>             → messages[1] (when not "start")
  ...
  if (webPath) {
    ...
    if (!checkExists(currentCwd)) {
      stderr.write(`[gwd] Project path does not exist: ${currentCwd}\n`)
      return {
        handled: true,
        exitCode: 1,
        action: 'start',
    ...
    stderr.write(`[gwd] Using project path: ${currentCwd}\n`)
  } else {

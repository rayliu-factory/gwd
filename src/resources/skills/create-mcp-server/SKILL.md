...
<context>
GWD consumes MCP heavily — see `src/resources/extensions/mcp-client/`, `src/resources/extensions/gwd/mcp-project-config.ts`, `src/resources/extensions/gwd/workflow-mcp.ts`, and `/gwd mcp` commands. Users frequently want to extend GWD with project-specific MCP servers (internal APIs, data sources, domain tools). This skill fills the authoring gap between "MCP exists" and "I have a working server."

...
- Project plugins: `.gwd/workflows/<name>.yaml` (preferred — checked into repo).
- Global plugins: `~/.gwd/workflows/<name>.yaml` (private to the machine). Use
  this when the user says "global" or "--global".
- Legacy location `.gwd/workflow-defs/<name>.yaml` still works but is being
...
7. Write to `.gwd/workflows/<name>.yaml` by default, or `~/.gwd/workflows/<name>.yaml`

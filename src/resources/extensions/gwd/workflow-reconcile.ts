...
import { clearPathCache, resolveGwdPathContract } from "./paths.js";
...
/**
 * Write a human-readable CONFLICTS.md to basePath/.gwd/CONFLICTS.md.
...
function _reconcileWorktreeLogsInner(
  ...
  const mainLogPath = join(mainBasePath, ".gwd", "event-log.jsonl");
  const wtLogPath = join(worktreeBasePath, ".gwd", "event-log.jsonl");
  ...
  if (conflicts.length > 0) {
    ...
    logError("reconcile", `${conflicts.length} conflict(s) detected on ${conflictSummary}${truncated}. Details: .gwd/CONFLICTS.md`, { count: String(conflicts.length), path: join(mainBasePath, ".gwd", "CONFLICTS.md") });
    return { autoMerged: 0, conflicts };
  }
  ...
  mkdirSync(join(mainBasePath, ".gwd"), { recursive: true });
  atomicWriteSync(join(mainBasePath, ".gwd", "event-log.jsonl"), logContent);

  // Step 8: Replay into DB (wrapped in a transaction by replayEvents)
  openDatabase(resolveGwdPathContract(mainBasePath).projectDb);
  replayEvents(merged);
...
export function listConflicts(basePath: string): ConflictEntry[] {
  const conflictsPath = join(basePath, ".gwd", "CONFLICTS.md");
...
export function resolveConflict(
  ...
  const mainLogPath = join(basePath, ".gwd", "event-log.jsonl");
  const wtLogPath = join(worktreeBasePath, ".gwd", "event-log.jsonl");
  const mainEvents = readEvents(mainLogPath);
  const wtEvents = readEvents(wtLogPath);
  const forkPoint = findForkPoint(mainEvents, wtEvents);
  ...
  openDatabase(resolveGwdPathContract(basePath).projectDb);

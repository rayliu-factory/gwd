// Migration transformer — converts parsed PlanningProject into GWDProject.
...
function buildTaskSummary(summary: PlanningSummary): GWDTaskSummaryData {
  return {
...
function mapTask(plan: PlanningPlan, index: number, summaries: Record<string, PlanningSummary>): GWDTask {
...
function buildSliceSummary(phase: PlanningPhase): GWDSliceSummaryData | null {
...
function mapRequirements(reqs: PlanningRequirement[]): GWDRequirement[] {
  let autoId = 0;
  return reqs.map((req) => {
    autoId++;
    return {
...
export function transformToGWD(parsed: PlanningProject): GWDProject {
  const milestones: GWDMilestone[] = [];
  ...
  if (isMultiMilestone) {
    // Multi-milestone mode: each roadmap milestone section → one GWDMilestone

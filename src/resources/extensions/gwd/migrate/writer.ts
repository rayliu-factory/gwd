...
// writeGWDDirectory: orchestrator that writes a complete .gwd directory tree from a GWDProject.
...
/** Result of writeGWDDirectory — lists all files that were written. */
...
/** Pre-write statistics computed from a GWDProject without I/O. */
 ...
 */
export function formatRoadmap(milestone: GWDMilestone): string {
  const lines: string[] = [];
 ...
 */
export function formatPlan(slice: GWDSlice): string {
  const lines: string[] = [];
 ...
 */
export function formatSliceSummary(slice: GWDSlice, milestoneId: string): string {
 ...
 * Output must parse correctly through parseSummary().
 */
export function formatTaskSummary(task: GWDTask, sliceId: string, milestoneId: string): string {
 ...
 */
export function formatTaskPlan(task: GWDTask, sliceId: string, milestoneId: string): string {
  const lines: string[] = [];
 ...
 */
export function formatRequirements(requirements: GWDRequirement[]): string {
  const lines: string[] = [];
  lines.push('# Requirements');
  lines.push('');

  const groups: Record<string, GWDRequirement[]> = {
    active: [],
    validated: [],
    deferred: [],
    'out-of-scope': [],
 ...
 */
export function formatState(milestones: GWDMilestone[]): string {
  const lines: string[] = [];
...
/**
 * Write a complete .gwd directory tree from a GWDProject.
 ...
 */
export async function writeGWDDirectory(
  project: GWDProject,
  targetPath: string,
): Promise<WrittenFiles> {
  const gwdDir = gwdRoot(targetPath);
  const milestonesBase = join(gwdDir, 'milestones');
  const paths: string[] = [];

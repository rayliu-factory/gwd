...
// Error-severity entries are persisted to .gwd/audit-log.jsonl (sanitized) for
...
export function readAuditLog(basePath?: string): LogEntry[] {
  ...
  if (!bp) return [];
  const auditPath = join(bp, ".gwd", "audit-log.jsonl");
...
function _push(
  ...
  // Persist errors to .gwd/audit-log.jsonl so they survive context resets.
  ...
  if (_auditBasePath && severity === "error") {
    try {
      const auditDir = join(_auditBasePath, ".gwd");

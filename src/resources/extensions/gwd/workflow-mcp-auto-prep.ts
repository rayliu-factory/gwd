...
export function prepareWorkflowMcpForProject(
  ...
  try {
    ...
    ctx.ui?.notify?.(
      `Claude Code MCP prep failed: ${err instanceof Error ? err.message : String(err)}. Detected Claude Code model but no workflow MCP. Please run /gwd mcp init . from your project root.`,
      "warning",
    );
    return null;
  }

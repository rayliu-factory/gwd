// Project/App: GWD-2
// File Purpose: GWD database facade, schema, migrations, and single-writer write API.
// GWD Database Abstraction Layer
...
// the engine database at `.gwd/gwd.db`. All other modules must call the
...
//
// The separate `.gwd/unit-claims.db` managed by `unit-ownership.ts` is an
...
/** Flush WAL into gwd.db so `git add .gwd/gwd.db` stages current state — safe while DB is open. */
...
/** List tasks with active escalation artifacts across a milestone (for /gwd escalate list). */
...
// All memory writes go through gwd-db.ts so the single-writer invariant
...
export function insertMemoryRow(args: {
   ...
   * `content` field. Used to retain decision-save fields (scope, decision,
   * choice, rationale, made_by, revisable) on architecture-category

...
const WORKTREE_PATH_SEGMENT = `${join(".gwd", "worktrees")}/`;

/**
 * Resolve the project root when running inside a `.gwd/worktrees/<name>/`
 * auto-worktree. Returns `null` if not in a worktree.
 *
 * Detection order:
 *   1. `GWD_WORKTREE` env var (set by the worktree launcher)
 *   2. `.gwd/worktrees/` segment in basePath
 */
function resolveWorktreeProjectRoot(basePath: string): string | null {
  ...
  // Everything before `.gwd/worktrees/` is the project root

...
import { _getAdapter, bulkInsertLegacyHierarchy } from "./gwd-db.js";
...
/**
 * Returns true when engine tables are empty AND a .gwd/milestones/ directory
...
export function needsAutoMigration(basePath: string): boolean {
  ...
  // Check if .gwd/milestones/ directory exists
...
/**
 * Migrate legacy markdown-only .gwd/ projects to engine DB state.
 * Reads .gwd/milestones/<ID>/ directories and parses ROADMAP.md, *-PLAN.md
...
export function migrateFromMarkdown(basePath: string): void {
  ...
  if (!existsSync(milestonesDir)) {
    process.stderr.write("workflow-migration: no .gwd/milestones/ directory found, nothing to migrate\n");
    return;
  }
  ...
  if (milestoneDirs.length === 0) {
    process.stderr.write("workflow-migration: no milestone directories found in .gwd/milestones/\n");
    return;
  }

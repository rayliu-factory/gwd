 ...
 *
 * Evidence is persisted to .gwd/safety/evidence-<mid>-<sid>-<tid>.json so it
 ...
 * Lives under .gwd/safety/ which is gitignored and session-scoped.
 */
function evidencePath(basePath: string, milestoneId: string, sliceId: string, taskId: string): string {
  return join(basePath, ".gwd", "safety", `evidence-${milestoneId}-${sliceId}-${taskId}.json`);
}

...
You are creating a GWD milestone from a provided specification document. This is a **headless** flow: do NOT ask the user questions. Wherever the interactive flow would ask, make your best judgment and document it as an assumption.
...
Before writing a roadmap, produce `.gwd/REQUIREMENTS.md`.
...
Directories use bare IDs. Files use ID-SUFFIX format. Titles live inside content, not names. Milestone dir: `.gwd/milestones/{{milestoneId}}/`; files: `{{milestoneId}}-CONTEXT.md`, `{{milestoneId}}-ROADMAP.md`; slice dirs: `S01/`, `S02/`, etc.
...
3. Write or update `.gwd/REQUIREMENTS.md` — use the **Requirements** output template below. Confirm states, ownership, and traceability before roadmap creation.
...
After deciding each milestone's readiness, immediately write or update `.gwd/DISCUSSION-MANIFEST.json`:

 ...
 * any changes to .gwd/, .planning/, and PLAN.md paths. Useful for
...
const EXCLUDED_PATHS = [".gwd", ".planning", "PLAN.md"] as const;
...
export async function handlePrBranch(
  ...
  if (commits.length === 0) {
    ctx.ui.notify("No code-only commits found (all commits only touch .gwd/ files).", "info");
    return;
  }

  if (dryRun) {
    const lines = [`Would create PR branch with ${commits.length} commits (filtering .gwd/ paths):\n`];
  ...
  try {
    ...
    ctx.ui.notify(
      `Created ${prBranch} with ${picked} commits${skippedMsg} (no .gwd/ artifacts).\nSwitch back: git checkout ${currentBranch}`,
      "success",
    );
  } catch (err) {

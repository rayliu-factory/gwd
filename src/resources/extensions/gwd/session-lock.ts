 ...
 *
 * The lock file (.gwd/auto.lock) contains JSON metadata (PID, start time,
 ...
 * to avoid contending on the shared `.gwd/auto.lock` (#2184).
 */
...
function ensureExitHandler(_gwdDir: string): void {
  ...
  process.once("exit", () => {
    ...
    // Lock files accumulate across main project .gwd/, worktree .gwd/,
...
export function acquireSessionLock(basePath: string): SessionLockResult {
  ...
  try {
    ...
    // In parallel worker mode, lockTarget is .gwd/parallel/<MID>/ (#2184).
    ...
    // Lock is held by another process — or the .gwd.lock/ directory is stranded.
...
export function releaseSessionLock(basePath: string): void {
  ...
  // main project .gwd/, worktree .gwd/, and projects registry paths.
  for (const dir of _lockDirRegistry) {
    const lockFile = join(dir, LOCK_FILE);
    const ownsRegisteredLock = isLockFileOwnedByCurrentProcess(lockFile);
    try {

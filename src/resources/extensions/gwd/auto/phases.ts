...
import { isDbAvailable, getMilestoneSlices } from "../gwd-db.js";
...
async function generateMilestoneReport(
  ...
  ctx.ui.notify(
    `Report saved: .gwd/reports/${basename(outPath)} — open index.html to browse progression.`,
    "info",
  );
}
 ...
 * leaked `gwd-preflight-stash:M00x:*` entries whenever `mergeAndExit` threw —

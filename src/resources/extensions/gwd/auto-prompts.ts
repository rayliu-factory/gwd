...
import { getPendingGates, getPendingGatesForTurn } from "./gwd-db.js";
import {
  GATE_REGISTRY,
  assertGateCoverage,
  getGatesForTurn,
...
export async function inlineDependencySummaries(
  ...
  try {
    const { isDbAvailable, getSlice } = await import("./gwd-db.js");
    if (isDbAvailable()) {
...
export async function inlineDecisionsFromDb(
  ...
  try {
    const { isDbAvailable } = await import("./gwd-db.js");
    if (isDbAvailable()) {
...
export async function checkNeedsReassessment(
  ...
  try {
    const { isDbAvailable, getMilestoneSlices } = await import("./gwd-db.js");
    if (isDbAvailable()) {
...
export async function buildValidateMilestonePrompt(
  ...
  try {
    const { isDbAvailable, getMilestone } = await import("./gwd-db.js");
    if (isDbAvailable()) {
  ...
  if (sid) {
    ...
    if (slicePlanPath) {
      ...
      if (tDir) {
        ...
        try {
          const { isDbAvailable, getSliceTasks } = await import("./gwd-db.js");
          if (isDbAvailable()) {

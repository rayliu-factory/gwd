...
import { openDatabase, closeDatabase, isDbAvailable, insertMilestone, insertSlice, insertTask, updateSliceStatus, upsertMilestonePlanning } from "../gwd-db.ts";
...
/**
 * Create a minimal .gwd/ directory structure with a task summary file.
 */
function createTaskSummaryOnDisk(basePath: string, mid: string, sid: string, tid: string): string {
  const tasksDir = join(basePath, ".gwd", "milestones", mid, "slices", sid, "tasks");
...
/**
 * Create a minimal .gwd/ directory structure with a slice summary file.
 */
function createSliceSummaryOnDisk(basePath: string, mid: string, sid: string): string {
  const sliceDir = join(basePath, ".gwd", "milestones", mid, "slices", sid);
...
function createRoadmapOnDisk(basePath: string, mid: string): string {
  const milestoneDir = join(basePath, ".gwd", "milestones", mid);
...
test("rogue detection: task summary on disk, no DB row → detected as rogue", () => {
  ...
  const dbPath = join(basePath, ".gwd", "gwd.db");

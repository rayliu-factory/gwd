 ...
 *
 * Verifies that when process.cwd() is inside a stale .gwd/worktrees/ path,
...
test("detects stale worktree path and extracts project root", () => {
  // Simulate the path pattern: /project/.gwd/worktrees/M004/...
  const projectRoot = "/Users/test/myproject";
  const stalePath = `${projectRoot}${sep}.gwd${sep}worktrees${sep}M004`;

  const marker = `${sep}.gwd${sep}worktrees${sep}`;
...
test("mergeMilestoneToMain restores cwd to project root", () => {
  ...
  try {
    ...
    const msDir = join(tempDir, ".gwd", "milestones", "M050");
...
test("process.cwd() inside removed worktree is recoverable", () => {
  ...
  try {
    ...
    // Create a .gwd/worktrees/M099 directory to simulate stale state
    const staleWtDir = join(tempDir, ".gwd", "worktrees", "M099");
    mkdirSync(staleWtDir, { recursive: true });

    // Enter the stale directory
    process.chdir(staleWtDir);
    const cwdBefore = process.cwd();
    assert.ok(cwdBefore.includes(`${sep}.gwd${sep}worktrees${sep}`), "cwd is inside worktree dir");

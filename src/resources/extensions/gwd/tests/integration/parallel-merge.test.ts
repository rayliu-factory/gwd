...
function createTempRepo(): string {
  ...
  // Mirror production: .gwd/worktrees/ is gitignored so autoCommitDirtyState
...
/**
 * Write `.gwd/preferences.md` with `git.isolation: branch` so the merge
...
function setupBranchIsolation(repo: string): void {
  ...
  try {
    run("git add .gwd/preferences.md", repo);
    run('git commit -m "test: add branch-isolation preferences"', repo);
  } catch { /* file may already be committed */ }
}

/**
 * Set up a milestone roadmap file in .gwd/milestones/<MID>/ AND commit it on
 * the current branch.
 *
...
function setupRoadmap(repo: string, mid: string, title: string, slices: string[]): void {
  const dir = join(repo, ".gwd", "milestones", mid);
  ...
  try {
    run(`git add .gwd/milestones/${mid}/${mid}-ROADMAP.md`, repo);
...
test("mergeCompletedMilestone — clean merge, session status cleaned up", async () => {
  ...
  try {
    ...
    writeSessionStatus(repo, {
      ...
      worktreePath: join(repo, ".gwd", "worktrees", "M010"),
    });
...
function setupCanonicalDbWithWorktree(basePath: string, mid: string): void {
  mkdirSync(join(basePath, ".gwd", "worktrees", mid), { recursive: true });

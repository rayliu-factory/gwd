...
describe('doctor-runtime', async () => {
  ...
  try {
    ...
    test('stale_crash_lock', async (t) => {
      ...
      // Leave DB open — runGWDDoctor's readCrashLock relies on the
      ...
      const { closeDatabase } = await import("../../gwd-db.ts");
      closeDatabase();
    });
...
`);
    ...
    test('orphaned_completed_units protected at fixLevel=task (#1809)', async () => {
      ...
      const taskLevelFix = await runGWDDoctor(dir, { fix: true, fixLevel: "task" });
      ...
      const allLevelFix = await runGWDDoctor(dir, { fix: true, fixLevel: "all" });
      assert.ok(

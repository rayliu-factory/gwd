...
function createTestRepo(): string {
  const repo = mkdtempSync(join(tmpdir(), "gwd-quick-lifecycle-"));
  run("git init -b main", repo);
  run(`git config user.name "GWD Test"`, repo);
  run(`git config user.email "test@gwd.dev"`, repo);
...
test('QUICK_BRANCH_RE: matches quick-task branches', () => {
  assert.ok(QUICK_BRANCH_RE.test("gwd/quick/1-fix-typo"), "matches standard quick branch");
});

  assert.ok(QUICK_BRANCH_RE.test("gwd/quick/42-some-long-slug-name"), "matches multi-digit quick branch");
  assert.ok(!QUICK_BRANCH_RE.test("main"), "rejects main");
  assert.ok(!QUICK_BRANCH_RE.test("gwd/M001/S01"), "rejects slice branch");
  assert.ok(!QUICK_BRANCH_RE.test("gwd/quickly-something"), "rejects non-quick prefix");
...
test('captureIntegrationBranch: skips quick-task branches', () => {
    ...
    run("git checkout -b gwd/quick/1-fix-typo", repo);
    assert.deepStrictEqual(getCurrentBranch(repo), "gwd/quick/1-fix-typo", "on quick branch");
...
test('captureIntegrationBranch: correct after quick branch round-trip', () => {
    ...
    run("git merge --squash gwd/quick/1-fix-typo", repo);
    run(`git commit -m "quick(Q1): fix-typo"`, repo);
    run("git branch -D gwd/quick/1-fix-typo", repo);
...
test('cleanupQuickBranch: merges back and cleans up (same session)', async () => {
    ...
    const returnState = {
      basePath: repo,
      originalBranch: "main",
      quickBranch: "gwd/quick/1-fix-typo",
      taskNum: 1,
      slug: "fix-typo",
      description: "fix typo",
    };
    const runtimeDir = join(repo, ".gwd", "runtime");
    ...
    assert.ok(!branches.includes("gwd/quick/1-fix-typo"), "quick branch deleted");
...
test('cleanupQuickBranch: recovers from disk state (cross-session)', async () => {
    ...
    run("git checkout -b gwd/quick/2-add-docs", repo);
    ...
    writeFileSync(join(runtimeDir, "quick-return.json"), JSON.stringify({
      basePath: repo,
      originalBranch: "main",
      quickBranch: "gwd/quick/2-add-docs",
      taskNum: 2,
      slug: "add-docs",

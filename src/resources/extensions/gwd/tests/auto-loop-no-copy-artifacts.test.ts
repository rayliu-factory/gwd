// gwd-2 + Phase C deletion regression: createAutoWorktree no longer copies .gwd/
//
// Verifies that createAutoWorktree on a project with a real (non-symlinked)
// .gwd/ does NOT populate .gwd/milestones/ inside the worktree. Pre-Phase-C,
// copyPlanningArtifacts would mirror the project-root .gwd/ into the
// worktree-local .gwd/. Phase C deleted that helper because writers in
// auto-mode now route through s.canonicalProjectRoot, so the worktree never
// needs a parallel .gwd/ projection.
...
test("createAutoWorktree does NOT copy project-root .gwd/milestones into the worktree", (t) => {
  ...
  // Phase C invariant: the worktree's .gwd/milestones/M001 must NOT exist
  // (no copyPlanningArtifacts), and the project-root version must still be
  // intact (it was the source, not destination).
  assert.equal(
    existsSync(join(wtPath, ".gwd", "milestones", "M001", "M001-CONTEXT.md")),
    false,
    "worktree should NOT have a copy of M001-CONTEXT.md (copyPlanningArtifacts deleted)",
  );
  assert.equal(
    existsSync(join(wtPath, ".gwd", "milestones", "M001", "M001-ROADMAP.md")),
    false,
    "worktree should NOT have a copy of M001-ROADMAP.md",
  );
  assert.equal(
    existsSync(join(base, ".gwd", "milestones", "M001", "M001-CONTEXT.md")),
    true,
    "project-root .gwd/ retains the canonical CONTEXT.md",
  );
});

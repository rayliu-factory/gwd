...
// registerGWDCommand was called inside the same try that loaded shortcuts.
//
// These tests exercise the real default export of index.ts (which calls
// registerGWDCommand via dynamic import, then attempts the full bootstrap)
...
describe("extension bootstrap isolation (#4168, #4172)", () => {
  ...
  test("degraded path: /gwd registered BEFORE any non-core command", async () => {
    ...
    // because index.ts awaits registerGWDCommand(pi) before importing
    // register-extension. Regression scenario: if a future refactor moves
    // registerGWDCommand into the try block or after other registrations,
...
// Behavioural contract for registerGwdExtension itself: each non-core
// registration is wrapped in its own try/catch so one failure does not
// prevent siblings from loading.

import { registerGwdExtension } from "../bootstrap/register-extension.ts";

describe("registerGwdExtension defensive registration", () => {
  test("a failing shortcut registration does not prevent kill command registration", async () => {
    ...
    // 'kill' is still registered, registerGwdExtension does not throw.
    ...
    assert.doesNotThrow(() => registerGwdExtension(pi as any));
    assert.ok(
  ...
  test("does NOT register /gwd (caller's responsibility, avoids double-registration)", () => {
    ...
    registerGwdExtension(pi as any);
    assert.ok(
      !registered.includes("gwd"),
      `registerGwdExtension must NOT register 'gwd' (it is registered separately by index.ts), got ${JSON.stringify(registered)}`,
    );
  });
});

  ...
  try {
    ...
    insertTask({
      ...
      planning: {
        ...
        inputs: ['src/resources/extensions/gwd/markdown-renderer.ts'],
    ...
    insertTask({
      ...
      planning: {
        ...
        estimate: '30m',
        files: ['src/resources/extensions/gwd/files.ts'],
        verify: 'node --test auto-recovery.test.ts',
        inputs: ['src/resources/extensions/gwd/files.ts'],
    ...
    assert.strictEqual(parsedPlan.tasks[0].files?.[0], 'src/resources/extensions/gwd/markdown-renderer.ts', 'files list preserved in slice plan');
...
test('── markdown-renderer: renderSliceSummary round-trip ──', async () => {
  ...
  try {
    ...
    const db = await import('../gwd-db.ts');

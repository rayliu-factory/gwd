...
function makeBaseRepo(): string {
  const base = mkdtempSync(join(tmpdir(), "gwd-2821-"));
  git(["init", "-b", "main"], base);
  git(["config", "user.name", "Test"], base);
  git(["config", "user.email", "test@test.com"], base);
  writeFileSync(join(base, "README.md"), "# test\n");
  mkdirSync(join(base, ".gwd", "milestones", "M011"), { recursive: true });
  git(["add", "."], base);
...
describe("#2821 Bug 1 — ASSESSMENT file force-synced on resume", () => {
  ...
  beforeEach(() => {
    mainBase = mkdtempSync(join(tmpdir(), "gwd-2821-main-"));
    wtBase = mkdtempSync(join(tmpdir(), "gwd-2821-wt-"));
    mkdirSync(join(mainBase, ".gwd", "milestones", "M011", "slices", "S01"), {
      recursive: true,
    });
    mkdirSync(join(wtBase, ".gwd", "milestones", "M011", "slices", "S01"), {
      recursive: true,
    });
  });
...
describe("#2821 Bug 2 — removeWorktree cleans up despite untracked files", () => {
  ...
  test("removes worktree directory even when it contains untracked files", () => {
    ...
    mkdirSync(
      join(info.path, ".gwd", "milestones", "M011", "slices", "S01"),
      { recursive: true },
    );
    writeFileSync(
      join(

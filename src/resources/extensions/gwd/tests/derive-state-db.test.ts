...
`;
  ...
  test('derive-state-db: replanning-slice via DB', async () => {
    ...
    try {
      ...
      const { _getAdapter } = await import('../gwd-db.ts');
...
`;
  ...
  test('derive-state-db: ghost milestone skipped when no DB row and no worktree', async () => {
    ...
    try {
      ...
      mkdirSync(join(base, '.gwd', 'milestones', 'M001'), { recursive: true });
      writeFileSync(join(base, '.gwd', 'milestones', 'M001', 'META.json'), '{}');
      // Real milestone
  ...
  test('derive-state-db: queued milestone row survives gwd_plan_milestone INSERT OR IGNORE', async () => {
    try {
      openDatabase(':memory:');

      // Simulates gwd_milestone_generate_id inserting a minimal queued row
      ...
      // Simulates gwd_plan_milestone calling insertMilestone (INSERT OR IGNORE)
  ...
  test('derive-state-db: queued milestone with worktree not flagged as ghost (#2921)', async () => {
    ...
    try {
      ...
      mkdirSync(join(base, '.gwd', 'milestones', 'M002', 'slices'), { recursive: true });

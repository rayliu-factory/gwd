...
// gwd_plan_slice and other DB tools to be missing from plan-slice subagent
// sessions.
//
// The behavioural invariant we can pin without grepping source: gwd_plan_slice
...
describe("#3616 — discuss tool scoping must not leak across sessions", () => {
	test("gwd_plan_slice is NOT in DISCUSS_TOOLS_ALLOWLIST", () => {
		assert.ok(
			!DISCUSS_TOOLS_ALLOWLIST.includes("gwd_plan_slice"),
			`gwd_plan_slice (a heavy planning tool) must be excluded from the discuss scope; allowlist=${JSON.stringify(DISCUSS_TOOLS_ALLOWLIST)}`,
		);
	});

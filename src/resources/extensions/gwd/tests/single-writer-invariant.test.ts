...
// route through a typed wrapper exported from gwd-db.ts.
//
// Allowlist:
...
// 1. Add a typed wrapper to gwd-db.ts that captures the SQL
...
const gwdDir = join(process.cwd(), "src/resources/extensions/gwd");
...
test("no module outside gwd-db.ts issues raw write SQL against the engine DB", () => {
  ...
  if (violations.length > 0) {
    ...
    assert.fail(
      `Found ${violations.length} raw write SQL bypass(es) outside gwd-db.ts:\n` +
        lines.join("\n") +
        "\n\nEach of these must be replaced with a typed wrapper exported from gwd-db.ts.",
    );
  }
});

test("gwd-db.ts exports the expected single-writer wrappers", async () => {
  ...
  for (const name of expected) {
    assert.ok(
      ...
      `gwd-db.ts must export ${name} as a function`,
    );
  }
});
...
test("the invariant test touches every .ts module under gwd/ (sanity check)", () => {
  ...
  const rels = files.map((f) => relative(gwdDir, f));
  assert.ok(rels.includes("gwd-db.ts"), "walker must include gwd-db.ts");

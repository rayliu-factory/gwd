 ...
 * mergeMilestoneToMain auto-resolves .gwd/ conflicts by accepting HEAD
...
test("#2766: stash pop conflict on .gwd/ files is auto-resolved", () => {
  ...
  try {
    ...
    // Modify .gwd/STATE.md on the milestone branch (diverges from main)
    writeFileSync(join(wtPath, ".gwd", "STATE.md"), "version: 2-milestone\n");
    run("git add .", wtPath);
    ...
    // Dirty .gwd/STATE.md in the main repo (stash will conflict on pop)
    writeFileSync(join(repo, ".gwd", "STATE.md"), "version: 2-main-dirty\n");

    const roadmap = makeRoadmap("M300", "Stash pop conflict test", [
      { id: "S01", title: "Feature" },
    ]);

    // mergeMilestoneToMain should succeed — .gwd/ conflict auto-resolved
    const result = mergeMilestoneToMain(repo, "M300", roadmap);
    assert.ok(
      result.commitMessage.includes("GWD-Milestone: M300"),
      "merge succeeds despite stash pop conflict on .gwd/ file",
    );
    ...
    assert.strictEqual(stashList, "", "stash is empty after .gwd/ conflict auto-resolution");
  } finally {
    cleanupTempRepo(repo);
  }
});

test("#2766: stash pop conflict on non-.gwd files preserves stash for manual resolution", () => {
  const repo = createTempRepo();
  try {
    ...
    assert.ok(
      ...
      "merge succeeds even with non-.gwd stash pop conflict",
    );
  } finally {
    cleanupTempRepo(repo);
  }

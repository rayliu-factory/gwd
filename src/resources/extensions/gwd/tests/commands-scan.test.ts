...
describe("buildScanOutputPaths", () => {
  test("returns paths under .gwd/codebase/ relative to basePath", () => {
    const base = "/some/project";
    const paths = buildScanOutputPaths("tech", base);
    assert.ok(paths.every((p) => p.startsWith(join(base, ".gwd", "codebase"))));
  });
...
describe("handleScan — handler integration", () => {
  test("creates .gwd/codebase/ directory", async (t) => {
    const tmpDir = makeTmpDir();
    ...
    assert.ok(existsSync(join(tmpDir, ".gwd", "codebase")));
  });
  ...
  test("warns when documents already exist", async (t) => {
    ...
    const codesbaseDir = join(tmpDir, ".gwd", "codebase");

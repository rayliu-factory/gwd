...
import { executeGwdGraph } from '../tools/memory-tools.ts';
...
test('memory-tools: gwd_graph returns LINK edges and filters by rel', () => {
  openDatabase(':memory:');
  ...
  const all = executeGwdGraph({ mode: 'query', memoryId: 'MEM001', depth: 1 });
  ...
  const filtered = executeGwdGraph({ mode: 'query', memoryId: 'MEM001', depth: 1, rel: 'related_to' });

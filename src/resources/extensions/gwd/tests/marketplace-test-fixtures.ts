...
export function getMarketplaceFixtures(testFileDir: string): { available: boolean; skipReason?: string; fixtures?: MarketplaceFixtureSet } {
  const gwd2Root = resolve(testFileDir, '../../../../..');
  const localClaudeSkillsPath = resolve(gwd2Root, '../claude_skills');
  const localClaudePluginsOfficialPath = resolve(gwd2Root, '../claude-plugins-official');
  ...
  try {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'gwd-marketplace-fixtures-'));

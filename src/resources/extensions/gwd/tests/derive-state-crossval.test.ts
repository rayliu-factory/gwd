...
// Proves both paths produce field-identical GWDState across 7 fixture scenarios,
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'gwd-crossval-'));
...
/**
 * Compare every GWDState field between DB and filesystem derivation.
 * prefix identifies the scenario in assertion messages.
 */
function assertStatesEqual(dbState: GWDState, fileState: GWDState, prefix: string): void {
  // Phase

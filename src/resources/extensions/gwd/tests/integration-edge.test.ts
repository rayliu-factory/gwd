...
import { openDatabase, closeDatabase, isDbAvailable, _resetProvider } from '../gwd-db.ts';
...
test('integration-edge: empty project', () => {
  ...
  const dbPath = join(gwdDir, 'test-edge-empty.db');
...
test('integration-edge: partial migration', () => {
  ...
  const dbPath = join(gwdDir, 'test-edge-partial.db');
...
test('integration-edge: fallback mode', () => {
  ...
  const dbPath = join(gwdDir, 'test-edge-fallback.db');

 ...
 * from the main repo's .gwd/ into the worktree's .gwd/ for the
 * specified milestone, and deletes gwd.db so it rebuilds from fresh state.
 *
 ...
 *   - gwd.db deleted in worktree after sync
...
describe('worktree-sync-milestones', async () => {
  ...
  console.log('\n=== 1. milestone directory synced from main to worktree ===');
  {
    ...
    try {
      ...
      assert.ok(!existsSync(join(wtBase, '.gwd', 'milestones', 'M001')), 'M001 missing before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.gwd', 'milestones', 'M001')), '#1311: M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.gwd', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.gwd', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 2. missing slices within milestone are synced ===');
  {
    ...
    try {
      ...
      const wtM001Dir = join(wtBase, '.gwd', 'milestones', 'M001');
      mkdirSync(join(wtM001Dir, 'slices', 'S01'), { recursive: true });
      writeFileSync(join(wtM001Dir, 'slices', 'S01', 'S01-PLAN.md'), '# S01 Plan');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.gwd', 'milestones', 'M001', 'slices', 'S02')), '#1311: S02 synced');
      assert.ok(existsSync(join(wtBase, '.gwd', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md')), 'S02 PLAN synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  // ─── 3. empty gwd.db deleted in worktree after sync ────────────────────
  console.log('\n=== 3. empty gwd.db deleted in worktree after sync ===');
  {
    const mainBase = createBase('main');
    const wtBase = createBase('wt');
    ...
    try {
      ...
      // Worktree has an empty (0-byte) gwd.db — stale/corrupt
      writeFileSync(join(wtBase, '.gwd', 'gwd.db'), '');
      assert.ok(existsSync(join(wtBase, '.gwd', 'gwd.db')), 'gwd.db exists before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(!existsSync(join(wtBase, '.gwd', 'gwd.db')), '#853: empty gwd.db deleted after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  // ─── 3b. non-empty gwd.db preserved in worktree after sync (#2815) ───
  console.log('\n=== 3b. non-empty gwd.db preserved in worktree after sync (#2815) ===');
  {
    const mainBase = createBase('main');
    const wtBase = createBase('wt');
    ...
    try {
      ...
      assert.ok(existsSync(join(wtBase, '.gwd', 'gwd.db')), '#2815: non-empty gwd.db preserved after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 7. milestones/ directory created in worktree when missing ===');
  {
    ...
    try {
      ...
      mkdirSync(join(wtBase, '.gwd'), { recursive: true });
  ...
  console.log('\n=== 8. syncWorktreeStateBack leaves task projections in worktree ===');
  {
    ...
    try {
      ...
      const wtSliceDir = join(wtBase, '.gwd', 'milestones', 'M002', 'slices', 'S01');
      ...
      mkdirSync(join(mainBase, '.gwd', 'milestones', 'M002'), { recursive: true });

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      const mainSliceDir = join(mainBase, '.gwd', 'milestones', 'M002', 'slices', 'S01');
  ...
  console.log('\n=== 9. syncWorktreeStateBack leaves root-level state projections authoritative ===');
  {
    ...
    try {
      ...
      mkdirSync(join(wtBase, '.gwd', 'milestones', 'M001'), { recursive: true });

      // Main has original REQUIREMENTS and PROJECT
      writeFileSync(join(mainBase, '.gwd', 'REQUIREMENTS.md'), '# Requirements\n## R001');
      writeFileSync(join(mainBase, '.gwd', 'PROJECT.md'), '# Project\n## Milestone: M001');

      // Worktree has updated versions (complete-milestone added M002 refs)
      writeFileSync(join(wtBase, '.gwd', 'REQUIREMENTS.md'), '# Requirements\n## R001\n## R002 — New req');
      writeFileSync(join(wtBase, '.gwd', 'PROJECT.md'), '# Project\n## Milestone: M001\n## Milestone: M002');
      writeFileSync(join(wtBase, '.gwd', 'KNOWLEDGE.md'), '# Knowledge\nLearned something.');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Root-level state projections must not be overwritten with worktree versions.
      const reqContent = readFileSync(join(mainBase, '.gwd', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        !reqContent.includes('R002'),
        'REQUIREMENTS.md ignores worktree projection content',
      );

      const projContent = readFileSync(join(mainBase, '.gwd', 'PROJECT.md'), 'utf-8');
      assert.ok(
      ...
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'KNOWLEDGE.md')),
  ...
  console.log('\n=== 10. syncWorktreeStateBack does not copy milestone dirs ===');
  {
    ...
    try {
      ...
      mkdirSync(join(wtBase, '.gwd', 'milestones'), { recursive: true });
      ...
      const wtM002Dir = join(wtBase, '.gwd', 'milestones', 'M002-abc123');
      ...
      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M001')),
        'M001 missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M002-abc123')),
      ...
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M001', 'M001-SUMMARY.md')),
      ...
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M002-abc123', 'M002-abc123-CONTEXT.md')),
        'M002 CONTEXT projection is not copied to main',
      );
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M002-abc123', 'M002-abc123-ROADMAP.md')),
  ...
  console.log('\n=== 11. complete-milestone worktree projections do not overwrite project root ===');
  {
    ...
    try {
      ...
      const mainM006 = join(mainBase, '.gwd', 'milestones', 'M006-589wvh');
      ...
      const wtM006 = join(wtBase, '.gwd', 'milestones', 'M006-589wvh');
      ...
      const wtM007 = join(wtBase, '.gwd', 'milestones', 'M007-wortc8');
      ...
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M006-589wvh', 'M006-589wvh-SUMMARY.md')),
        'M006 SUMMARY NOT synced (current milestone skipped)',
      );

      // Verify M007 worktree projections are not copied back.
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M007-wortc8', 'M007-wortc8-CONTEXT.md')),
        'M007 CONTEXT projection is not copied to main',
      );
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'M007-wortc8', 'M007-wortc8-ROADMAP.md')),
  ...
  console.log('\n=== 12. root files not in worktree are not created in main ===');
  {
    ...
    try {
      ...
      writeFileSync(join(mainBase, '.gwd', 'REQUIREMENTS.md'), '# Original');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Main's REQUIREMENTS should be untouched (worktree had nothing to sync)
      const content = readFileSync(join(mainBase, '.gwd', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        content === '# Original',
  ...
  console.log('\n=== 13. QUEUE.md skipped; completed-units.json diagnostic synced ===');
  {
    ...
    try {
      ...
      writeFileSync(join(wtBase, '.gwd', 'QUEUE.md'), '# Queue\n- M002 next');
      writeFileSync(
        join(wtBase, '.gwd', 'completed-units.json'),
        JSON.stringify({ units: [{ id: 'M001-S01-T01', completed: true }] }),
      );

      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'QUEUE.md')),
        'QUEUE.md missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'completed-units.json')),
      ...
      const cuContent = readFileSync(join(mainBase, '.gwd', 'completed-units.json'), 'utf-8');
      assert.ok(
  ...
  console.log('\n=== 14. syncGsdStateToWorktree syncs non-standard milestone dir names (#1547) ===');
  {
    ...
    try {
      ...
      const customDir = join(mainBase, '.gwd', 'milestones', 'sprint-alpha');
      mkdirSync(customDir, { recursive: true });
      writeFileSync(join(customDir, 'CONTEXT.md'), '# Sprint Alpha Context');

      const suffixDir = join(mainBase, '.gwd', 'milestones', 'M001-abc123');
      ...
      assert.ok(
        existsSync(join(wtBase, '.gwd', 'milestones', 'M001-abc123', 'M001-abc123-CONTEXT.md')),
  ...
  console.log('\n=== 15. syncWorktreeStateBack skips non-standard milestone dir names ===');
  {
    ...
    try {
      ...
      const wtCustomDir = join(wtBase, '.gwd', 'milestones', 'sprint-beta');
      mkdirSync(wtCustomDir, { recursive: true });
      writeFileSync(join(wtCustomDir, 'SUMMARY.md'), '# Sprint Beta Summary');

      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'sprint-beta')),
      ...
      assert.ok(
        !existsSync(join(mainBase, '.gwd', 'milestones', 'sprint-beta', 'SUMMARY.md')),

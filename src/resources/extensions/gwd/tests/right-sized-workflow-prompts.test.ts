...
test("workflow docs no longer contain blanket 4-10 slice guidance", () => {
  const docs = readFileSync(join(process.cwd(), "src", "resources", "GWD-WORKFLOW.md"), "utf-8");
...
test("prompt templates carry right-sized planning and closeout mode guidance", () => {
  const planTemplate = readFileSync(join(process.cwd(), "src", "resources", "extensions", "gwd", "prompts", "plan-milestone.md"), "utf-8");
  const completeTemplate = readFileSync(join(process.cwd(), "src", "resources", "extensions", "gwd", "prompts", "complete-milestone.md"), "utf-8");

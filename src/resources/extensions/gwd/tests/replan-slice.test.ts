...
console.log('\n=== prompt: replan-slice template loads and substitutes variables ===');
{
  ...
  assert.ok(prompt.includes('.gwd/milestones/M001/slices/S01/S01-PLAN.md'), 'prompt contains planPath');
...
console.log('\n=== prompt: replan-slice contains preserve-completed-tasks instruction ===');
{
  const prompt = loadPromptFromWorktree('replan-slice', {
    ...
    planPath: '.gwd/milestones/M001/slices/S01/S01-PLAN.md',
    blockerTaskId: 'T01',
    replanPath: '.gwd/milestones/M001/slices/S01/S01-REPLAN.md',
    inlinedContext: '',
  });
...
console.log('\n=== doctor: blocker + no REPLAN.md → blocker_discovered_no_replan issue ===');
{
  ...
  const report = await runGWDDoctor(base, { fix: false, scope: 'M001/S01' });

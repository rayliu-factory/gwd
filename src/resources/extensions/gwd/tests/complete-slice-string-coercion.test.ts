...
describe("handleCompleteSlice with coerced string arrays (#3565)", () => {
  ...
  beforeEach(() => {
    dbPath = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "gwd-coerce-")),
      "test.db",
    );
    openDatabase(dbPath);

    basePath = fs.mkdtempSync(path.join(os.tmpdir(), "gwd-coerce-handler-"));
    const sliceDir = path.join(basePath, ".gwd", "milestones", "M001", "slices", "S01", "tasks");
    fs.mkdirSync(sliceDir, { recursive: true });

    const roadmapPath = path.join(basePath, ".gwd", "milestones", "M001", "M001-ROADMAP.md");
    fs.writeFileSync(
      roadmapPath,
      [
        "# M001: Test Milestone",

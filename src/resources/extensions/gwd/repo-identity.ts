 ...
 *   3. There is no *project* `.gwd` directory at the git root or any
 ...
 *
 * When the git root already has a project `.gwd`, the directory is a
...
export function isInheritedRepo(basePath: string): boolean {
  try {
    ...
    // .gwd at basePath means GWD state is set up for THIS project, which
...
/**
 * Distinguish a *project* `.gwd` from the global `~/.gwd` state directory.
 *
 * A project `.gwd` is either:
 ...
 * `~/.gwd` exists but is the global state directory — not a project `.gwd`.
...
function isProjectGsd(gsdPath: string): boolean {
  ...
  try {
    ...
    // lstat failed — treat as no .gwd present
  }
 ...
 * Local repos use a `.gwd-id` marker file for recovery after moves.
 *
...
/**
 * macOS collision pattern: `.gwd 2`, `.gwd 3`, `.gwd 4`, etc.
 *
 * When `symlinkSync` (or Finder) tries to create `.gwd` but a real directory
 * already exists at that path, macOS APFS silently renames the new entry to
 * `.gwd 2`, then `.gwd 3`, and so on. These numbered variants confuse GWD
 * because the canonical `.gwd` path no longer resolves to the external state
...
/**
 * Write a `.gwd-id` marker file in the project root.
 *
 ...
 * checks the `.gwd-id` marker for the original identity hash and recovers
...
/**
 * Ensure the `<project>/.gwd` symlink points to the external state directory.
 *
 * 1. Clean up any macOS numbered collision variants (`.gwd 2`, `.gwd 3`, etc.)
 * 2. Resolve external dir (with relocation recovery via `.gwd-id` marker)
 * 3. mkdir -p the external dir
 * 4. If `<project>/.gwd` doesn't exist → create symlink
 * 5. If `<project>/.gwd` is already the correct symlink → no-op
 * 6. If `<project>/.gwd` is a real directory → return as-is (migration handles later)
 * 7. Write `.gwd-id` marker for future relocation recovery
 *
...
function ensureGsdSymlinkCore(projectPath: string): string {
  ...
  // not a project .gwd. This can happen if resolveProjectRoot() or
  ...
  // repo that already has a .gwd at the git root, do not create a duplicate
  // symlink in the subdirectory — that causes `.gwd 2` collision variants on
  // macOS (#2380). Worktrees are excluded because they legitimately need their
  // own .gwd symlink pointing at the shared external state dir.
  if (!inWorktree) {
    try {

Debug GWD itself. Trace the symptom to root cause in current source and produce a filing-ready GitHub issue with file:line references and a concrete fix suggestion.
...
Then **offer GitHub issue creation**: "Would you like me to create a GitHub issue for this on rayliu-factory/gwd?"

**CRITICAL:** The `github_issues` tool targets only the current user's repository and has no `repo` parameter. Use `gh issue create --repo rayliu-factory/gwd` via `bash`. Do NOT use the `github_issues` tool.

If yes, create using the `bash` tool:

```bash
ISSUE_BODY_FILE="${TMPDIR:-${TEMP:-${TMP:-.}}}/gwd-forensic-issue.md"
cat > "$ISSUE_BODY_FILE" << 'GWD_ISSUE_BODY'
## Problem
[1-2 sentence summary]

## Root Cause
[Specific file:line in GWD source, with code snippet showing the bug]

## Expected Behavior
[What the code should do instead — concrete fix suggestion]

## Environment
- GWD version: [from report]
- Model: [from report]
...
---
*Auto-generated by `/gwd forensics`*
GWD_ISSUE_BODY

ISSUE_URL=$(gh issue create --repo rayliu-factory/gwd \
  --title "..." \
...
ISSUE_ID=$(gh api graphql -f query='{ repository(owner:"rayliu-factory",name:"gwd") { issue(number:'"$ISSUE_NUM"') { id } } }' --jq '.data.repository.issue.id')
TYPE_ID=$(gh api graphql -f query='{ repository(owner:"rayliu-factory",name:"gwd") { issueTypes(first:20) { nodes { id name } } } }' --jq '.data.repository.issueTypes.nodes[] | select(.name=="Bug") | .id')
...
- Do not include user project code — only GWD structure (tool names, file names, error messages)

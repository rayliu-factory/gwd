...
test("create-gwd-extension docs do NOT direct users to install in ~/.gwd/agent/extensions/", async (t) => {
  for (const { file, label } of docsToCheck) {
    await t.test(`${label} does not tell users to place extensions in ~/.gwd/agent/extensions/`, () => {
      ...
      // ~/.gwd/agent/extensions/ should only appear in context that clearly marks
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes("~/.gwd/agent/extensions/")) {
          // If the line references ~/.gwd/agent/extensions/, it must be in a
          ...
          assert.ok(
            isBundledContext,
            `${label} line ${i + 1} references ~/.gwd/agent/extensions/ without ` +

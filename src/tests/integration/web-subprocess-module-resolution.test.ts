...
test("isUnderNodeModules returns true for Unix paths under node_modules/", () => {
  assert.equal(
    isUnderNodeModules("/usr/lib/node_modules/@appfiex-rayliu/gwd"),
    true,
  )
})
...
test("resolveSubprocessModule returns source .ts path when NOT under node_modules", () => {
  ...
  assert.deepEqual(result, {
    modulePath: join(packageRoot, "src", "resources/extensions/gwd/workspace-index.ts"),
    useCompiledJs: false,
  })
})

test("resolveSubprocessModule returns compiled .js path when under node_modules and dist file exists", () => {
  const packageRoot = "/usr/lib/node_modules/@appfiex-rayliu/gwd"
  const distPath = join(packageRoot, "dist", "resources/extensions/gwd/workspace-index.js")
  const result = resolveSubprocessModule(
    packageRoot,
    "resources/extensions/gwd/workspace-index.ts",
    (p: string) => p === distPath,
...
test("resolveSubprocessModule handles Windows paths under node_modules", () => {
  ...
  const distPath = join(packageRoot, "dist", "resources/extensions/gwd/auto.js")
  const result = resolveSubprocessModule(
    packageRoot,
    "resources/extensions/gwd/auto.ts",
    (p: string) => p === distPath,
  )
...
test("resolveSubprocessModule strips .ts extension when building dist .js path", () => {
  ...
  resolveSubprocessModule(
    packageRoot,
    "resources/extensions/gwd/doctor.ts",
    (p: string) => { checkedPath = p; return true },
  )

  assert.equal(
    checkedPath,
    join(packageRoot, "dist", "resources/extensions/gwd/doctor.js"),
...
test("buildSubprocessPrefixArgs omits TS loaders when compiled JS was selected", () => {
  assert.deepEqual(
    buildSubprocessPrefixArgs(
      "/usr/lib/node_modules/@appfiex-rayliu/gwd",
      {
        modulePath: "/usr/lib/node_modules/@appfiex-rayliu/gwd/dist/resources/extensions/gwd/workspace-index.js",
        useCompiledJs: true,
      },
      "file:///loader.mjs",
    ),

...
async function collectRecoveryDiagnosticsChildPayload(
  ...
  const script = [
    ...
    'const doctor = await import(pathToFileURL(process.env.GWD_RECOVERY_DOCTOR_MODULE).href);',
    'const forensics = await import(pathToFileURL(process.env.GWD_RECOVERY_FORENSICS_MODULE).href);',
    'const basePath = process.env.GWD_RECOVERY_BASE;',
    'const scope = process.env.GWD_RECOVERY_SCOPE || undefined;',
    'const unitType = process.env.GWD_RECOVERY_UNIT_TYPE || "execute-project";',
  ...
  return await new Promise<RecoveryDiagnosticsChildPayload>((resolveResult, reject) => {
    execFile(
      ...
      ],
      {
        ...
        env: {
          ...env,
          GWD_RECOVERY_BASE: basePath,
          GWD_RECOVERY_SCOPE: scope ?? "",

...
Welcome to the GWD documentation. This covers everything from getting started to advanced configuration, auto-mode internals, and extending GWD with the Pi SDK.

## User Documentation

Guides for installing, configuring, and using GWD day-to-day. Located in [`user-docs/`](./user-docs/).
...
| [Import `.planning` Projects](./user-docs/migration.md) | Import legacy `.planning` directories into GWD |
| [Troubleshooting](./user-docs/troubleshooting.md) | Common issues, `/gwd doctor` real-time visibility, `/gwd forensics` full debugger, and recovery procedures |
| [Web Interface](./user-docs/web-interface.md) | Browser-based project management with `gwd --web` |
| [VS Code Extension](../vscode-extension/README.md) | Chat participant, sidebar dashboard, and RPC integration for VS Code |
| [Release Notes](../CHANGELOG.md) | Release notes and version history |
...
| [Contributor Architecture and Data Flow](./dev/architecture-data-flow.md) | Contributor-oriented repo map, runtime data flow, and agent change paths |
| [Native Engine](../native/README.md) | Rust N-API modules for performance-critical operations |
| [ADR-001: Branchless Worktree Architecture](./dev/ADR-001-branchless-worktree-architecture.md) | Decision record for git architecture |
...
| [ADR-008: GWD Tools over MCP](./dev/ADR-008-gwd-tools-over-mcp-for-provider-parity.md) | Native tools over MCP for provider parity |
...
Guides for the underlying Pi SDK that GWD is built on. Located in [`dev/`](./dev/).

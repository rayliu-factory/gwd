...
**Related ADR:** [ADR-008-gwd-tools-over-mcp-for-provider-parity.md](./ADR-008-gwd-tools-over-mcp-for-provider-parity.md)
**Status:** Draft
**Date:** 2026-04-09

## Objective

Implement the ADR-008 decision by exposing the core GWD workflow tool contract over MCP, then wiring MCP-backed access into provider paths that cannot use the native in-process GWD tool registry directly.

The first usable outcome is:

- a Claude Code-backed execution session can complete a task using canonical GWD tools
- no manual summary-writing fallback is needed
- native provider behavior remains unchanged

## Non-Goals

- Replacing native in-process GWD tools with MCP
...
- Canonical GWD state transitions must remain DB-backed
...
- `src/resources/extensions/gwd/bootstrap/db-tools.ts`
- `src/resources/extensions/gwd/bootstrap/query-tools.ts`
- `src/resources/extensions/gwd/tools/complete-task.ts`
...
Goal: add an MCP server surface for real GWD workflow tools, distinct from the current session/read API.
...
- extend existing MCP package vs create `packages/mcp-gwd-tools-server`
...
Goal: attach the GWD workflow-tool MCP surface to Claude Code sessions.
...
- build a GWD-managed `mcpServers` config for the Claude SDK session
- attach the workflow MCP server only when the session requires GWD tools
- keep current Claude Code streaming behavior intact

Exit criteria:

- Claude Code session can discover the GWD workflow MCP tools
- task execution path can call `gwd_task_complete` successfully
...
- GWD dispatch / auto-mode preflight
- provider selection and routing checks
- user-facing compatibility errors

Required behavior:

- if native GWD tools are available, proceed
- else if GWD workflow MCP tools are available, proceed
...
- `src/resources/extensions/gwd/prompts/execute-task.md`
- related planning/discuss prompts that reference tool availability
- provider and MCP docs

Rules:

- prompts should keep requiring canonical GWD completion/planning tools
...
- extract shared logic for `gwd_summary_save`, `gwd_task_complete`, and `gwd_milestone_status`
...
- Claude Code can call `gwd_task_complete`
...
- plan or execute a small fixture task and complete it through canonical GWD tools
...
1. Claude Code-backed execution can use canonical GWD workflow tools over MCP.
...
1. Extract shared handlers for `gwd_summary_save`, `gwd_task_complete`, and `gwd_milestone_status`.

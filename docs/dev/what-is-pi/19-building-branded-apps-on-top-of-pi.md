...
- **No, you do not have to rely on `~/.gwd`** if you embed pi with custom paths and storage
...
```bash
npm install -g my-gwd
# or a standalone binary / packaged desktop app

gwd
```

And inside `gwd`, you import pi packages and create your own session, UI, storage, and resource loading behavior.

---

## 19.3 The Second Biggest Misconception: `~/.gwd` Is a Default, Not a Requirement

Pi CLI defaults to `~/.gwd/agent`, but embedded applications are not forced to use it.
...
- `~/.gwd/agent`
- `~/Library/Application Support/GWD`
- `%APPDATA%/GWD`
...
This is the most natural fit for tools like `gwd`.
...
- easiest way to remove dependency on `~/.gwd`
...
|------|---------------------|
| Branded CLI like `gwd` | `@mariozechner/pi-coding-agent` SDK |
...
## 19.6 The Recommended Path for a Branded CLI Like `gwd`
...
const appRoot = path.join(os.homedir(), ".gwd");
...
This is another place where people often assume they must rely on discovery from `~/.gwd` or `.gwd/`.
  ...
  extensionFactories: [
    (pi) => {
      pi.registerCommand("hello", {
        ...
        handler: async (_args, ctx) => ctx.ui.notify("Hello from GWD", "info"),
      });
    },
  ],
});
...
const loader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () => "You are GWD, a specialized software delivery agent.",
});
```
...
### Recommendation
For a branded tool like `gwd`, default to **bundled-app product** behavior.
...
```typescript
const authStorage = AuthStorage.create("/path/to/gwd/auth.json");
```
...
```typescript
const modelRegistry = new ModelRegistry(authStorage, "/path/to/gwd/models.json");
```
...
## 19.18 Building a Branded `gwd` CLI: Recommended Shape
...
const resourceLoader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () =>
    "You are GWD, a branded software delivery agent. Prefer project-specific workflows and terminology.",
  additionalExtensionPaths: [
...
If your app silently loads from a user's `~/.gwd`, you may get:
...
If your app is called `gwd`, but state lives in `~/.gwd`, users will notice.
...
- Good for productized tools like `gwd`
...
> “I want users to download and run `gwd`, and have it use pi internally without requiring a separate pi install or `~/.gwd` setup.”

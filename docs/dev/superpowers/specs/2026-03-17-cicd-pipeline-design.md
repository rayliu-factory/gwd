# CI/CD Pipeline Design — GWD

## Overview

A three-stage promotion pipeline for GWD that moves merged PRs through Dev → Test → Prod using npm dist-tags as environment markers, GitHub Environments for approval gates, and Docker images for both CI acceleration and end-user distribution.
...
5. End users can run GWD via Docker as an alternative to npm
...
Dev versions (`@dev` tag) use the native binaries from the most recent stable `build-native.yml` release. The `optionalDependencies` in `package.json` use `>=` ranges, so a `-dev.` version of `gwd-pi` resolves the latest stable `@appfiex-rayliu/gwd-engine-*` packages from the registry.
...
- **Name:** `ghcr.io/gwd-build/gwd-ci-builder`
...
- **Name:** `ghcr.io/gwd-build/gwd-pi`
- **Base:** `node:22-slim`
- **Contains:** Node 22, git, `gwd-pi` installed globally
- **Size:** ~250 MB
- **Tags:** `:latest`, `:next`, `:v2.27.0`
- **Published:** On every Prod promotion
- **Purpose:** `docker run ghcr.io/gwd-build/gwd-pi` as alternative to `npx`
...
The `FixtureProvider` implements the `Provider` interface from `@gwd/pi-ai` (the same interface all 20+ built-in providers implement). It registers itself via environment variable detection at provider initialization:

```typescript
// Pseudocode — actual implementation will follow pi-ai patterns
import type { Provider, StreamingResponse } from "@gwd/pi-ai";
...
- **Registration:** When `GWD_FIXTURE_MODE` is set, the fixture provider wraps the configured real provider. No changes to provider selection logic needed.
...
|------|---------|----------|
| **Record** | `GWD_FIXTURE_MODE=record GWD_FIXTURE_DIR=./fixtures` | Wraps real provider, saves request/response pairs |
| **Replay** | `GWD_FIXTURE_MODE=replay GWD_FIXTURE_DIR=./fixtures` | Returns saved responses, zero API calls |
...
5. `docker run ghcr.io/gwd-build/gwd-pi --version` returns the correct version

...
**解决：** 先运行 `/gwd doctor` 修复状态，然后执行 `/gwd auto` 恢复。如果问题持续存在，检查预期 artifact 文件是否确实已经写到磁盘。

### 自动模式因 “Loop detected” 停止

**原因：** 同一个单元连续两次没有生成预期 artifact。

**解决：** 检查 task plan 是否足够清晰。如果 plan 存在歧义，先手动澄清，再执行 `/gwd auto` 恢复。
...
对于瞬时错误，GWD 会短暂停顿后自动继续。对于永久性错误，建议配置 fallback models：
...
**Headless 模式：** `gwd headless auto` 在进程崩溃时会自动重启整个进程（默认 3 次，带指数退避）。与 provider 错误自动恢复配合后，能支持真正的夜间无人值守运行。
...
**解决：** 提高偏好设置中的 `budget_ceiling`，或者切换到 `budget` token profile 降低每个工作单元成本，然后再执行 `/gwd auto` 恢复。

### 过期锁文件

**症状：** 自动模式无法启动，提示另一个会话正在运行。

**解决：** GWD 会自动检测过期锁：如果持有锁的 PID 已死亡，则在下次 `/gwd auto` 时清理并重新获取锁。它也会处理 `proper-lockfile` 崩溃后遗留的 `.gwd.lock/` 目录。如果自动恢复失败，可手动删除 `.gwd/auto.lock` 和 `.gwd.lock/`：
...
**症状：** Worktree merge 在 `.gwd/` 文件上失败。

**解决：** GWD 会自动解决 `.gwd/` 运行时文件上的冲突。对于代码文件的内容冲突，LLM 会先获得一次 fix-merge 会话进行自动修复；若失败，则需要手动解决。

### Pre-dispatch 提示 milestone integration branch 已不存在

**症状：** 自动模式或 `/gwd doctor` 报告某个 milestone 记录的 integration branch 已经不在 git 中。

**这意味着什么：** 该 milestone 的 `.gwd/milestones/<MID>/<MID>-META.json` 里仍然记录着启动时的 branch，但该 branch 之后被重命名或删除了。

**当前行为：**

- 如果 GWD 能确定性地恢复到一个安全 branch，就不会再直接 hard-stop 自动模式
- 安全回退的顺序是：
  - 显式配置且存在的 `git.main_branch`
  - 仓库自动检测到的默认 integration branch（例如 `main` 或 `master`）
- 在这种情况下，`/gwd doctor` 会给出 warning，而 `/gwd doctor fix` 会把过期的 metadata 改写为当前有效 branch
- 如果无法确定安全回退 branch，GWD 仍会阻止继续运行

**解决：**

- 先执行 `/gwd doctor fix`，在安全回退很明显时自动改写过期 metadata
- 如果 GWD 仍然阻塞，则请重新创建缺失 branch，或更新 git 偏好设置，让 `git.main_branch` 指向一个真实存在的 branch

### 写 `.gwd/` 文件时出现瞬时 `EBUSY` / `EPERM` / `EACCES`

**症状：** 在 Windows 上，自动模式或 doctor 在更新 `.gwd/` 文件时偶发 `EBUSY`、`EPERM` 或 `EACCES`。

**原因：** 杀毒软件、索引器、编辑器或文件监视器可能会在 GWD 执行原子 rename 的瞬间，短暂锁住目标文件或临时文件。

**当前行为：** GWD 现在会对这类瞬时 rename 失败做短时、有上界的退避重试；这样既能覆盖短暂锁竞争，也不会因为真正的文件系统问题而无限挂起。

**解决：**

- 重新执行操作；大多数瞬时锁竞争会很快自行解除
- 如果错误持续，关闭可能占用该文件的工具后再试
- 如果反复失败，运行 `/gwd doctor`，确认仓库状态依旧健康，并记录具体路径与错误码

### Node v24 Web 启动失败

**症状：** 在 Node v24 上执行 `gwd --web` 时，报 `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`。
...
**症状：** `gwd --web` 因端口 3000 已被占用而失败，但实际上并没有运行中的 GWD 会话。

**原因：** 上一次 Web server 退出时未能清理进程。

**解决：** 已在 v2.42.0+ 修复。现在 GWD 会自动清理过期的 Web server 进程。如果你还在旧版本，可手动终止孤儿进程：`lsof -ti:3000 | xargs kill`。
...
- 当前项目里不存在 `.mcp.json` 或 `.gwd/mcp.json`
- 配置文件不是合法 JSON
- 你是在另一个项目目录中配置的 server，但当前启动 GWD 的目录不同

**解决：**

- 把 server 配置加到 `.mcp.json` 或 `.gwd/mcp.json`
- 确认文件能被正常解析为 JSON
...
- 在 GWD 外部直接运行该命令，确认 server 能真正启动
- 检查后端 URL 或依赖服务是否可达
...
- 如果你正在开发 GWD 本身，在 schema 变更后重新执行 `npm run build`

### 本地 stdio server 手动可用，但在 GWD 中不可用

**症状：** 手动执行 server 命令没有问题，但 GWD 连接不上。

**常见原因：**

- Server 依赖某些 GWD 不会继承的 shell 状态
...
### Session lock 被另一个终端中的 `/gwd` 抢走

**症状：** 在第二个终端运行 `/gwd`（step mode）时，正在运行的自动模式会话失去了锁。

**解决：** 已在 v2.36.0 修复。现在裸 `/gwd` 不会再从运行中的自动模式会话手里抢 session lock。升级到最新版本。
...
- **Dashboard：** `Ctrl+Alt+G` 或 `/gwd status`，查看实时诊断信息
- **Forensics：** `/gwd forensics`，用于对自动模式失败做结构化事后分析
- **Session logs：** `.gwd/activity/` 中包含用于崩溃取证的 JSONL 会话转储

## iTerm2 专属问题

### Ctrl+Alt 快捷键触发了错误动作（例如 Ctrl+Alt+G 打开了外部编辑器，而不是 GWD dashboard）

**症状：** 按下 Ctrl+Alt+G 后，会触发外部编辑器提示（Ctrl+G），而不是 GWD dashboard。其它 Ctrl+Alt 快捷键也表现得像它们对应的 Ctrl-only 快捷键。
...
**症状：** `gwd_decision_save`（及其别名 `gwd_save_decision`）、`gwd_requirement_update`（及其别名 `gwd_update_requirement`）或 `gwd_summary_save`（及其别名 `gwd_save_summary`）报这个错误。

**原因：** SQLite 数据库未初始化。这个问题会出现在 v2.29 之前的手动 `/gwd` 会话（非自动模式）中。
...
GWD 会根据项目文件自动检测 language servers（例如 `package.json` → TypeScript、`Cargo.toml` → Rust、`go.mod` → Go）。如果没有检测到 server，agent 会跳过 LSP 功能。
...
安装完成后，执行 `lsp reload` 即可重新检测，无需重启 GWD。
...
**原因：** GWD 在 macOS 上会把 `osascript display notification` 作为回退方案。这个命令的通知归属你的终端应用（Ghostty、iTerm2、Alacritty、Kitty、Warp 等）。如果该终端应用在 System Settings → Notifications 中没有权限，macOS 会静默丢弃通知，而 `osascript` 仍然返回 0，不会报错。
...
GWD 在检测到 `terminal-notifier` 可用时会自动优先使用它。首次使用时，macOS 会弹出通知权限请求，这是预期行为。

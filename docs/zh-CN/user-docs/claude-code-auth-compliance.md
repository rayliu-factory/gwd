...
2. 永远不要让 GWD 用户通过 GWD 托管的 Anthropic OAuth 去登录 Claude 订阅。
3. 永远不要把 Claude.ai 的订阅 OAuth 凭据交换成 bearer token，然后冒充 Claude Code 直接调用 Anthropic API。
4. 如果 GWD 需要直接访问 Anthropic API，则必须要求使用 Claude Console API key、Bedrock、Vertex 或其他被明确支持的 provider 路径。
...
- 如果 GWD 是通过 shell 调用或嵌入 Claude Code，那么它应尊重 Claude Code 自己的凭据选择逻辑，而不是再发明一套平行的 Anthropic OAuth 流程
...
- 如果 GWD 作为一个产品面向用户提供 Anthropic 能力，那么任何直接 Anthropic 访问都应走商业认证路径，而不是复用订阅 token
...
- 由 GWD 托管的 Anthropic 订阅 OAuth 流程属于高风险
- 在 GWD 自己的 API client 中复用用户 Claude 订阅凭据属于高风险
- 任何会让 Anthropic 误以为请求来自 Claude Code、但实际上来自 GWD 基础设施的流程，都越界了
...
这些方向是正确的，因为此时 GWD 使用的是用户自己本地安装的 Claude Code，作为已认证的 Anthropic surface。
...
- GWD 不得为终端用户实现自己的 Anthropic 订阅 OAuth 流程
- GWD 不得持久化 Anthropic 订阅 OAuth token，供后续 API 调用使用
- GWD 不得使用由 GWD 获取的订阅 OAuth tokens 来发送 Anthropic API 流量
- GWD 可以支持 Anthropic 直接访问，但仅限以下方式：
  - `ANTHROPIC_API_KEY`
...
4. 增加明确的 UI 文案：
   - “Claude 订阅用户：请登录本地 Claude Code app / CLI，而不是 GWD。”
5. 阻止任何把 Anthropic OAuth 凭据转换成 GWD 托管请求 API 认证的迁移或代码路径
...
如果某个拟议中的 GWD 特性需要访问 Anthropic，先问一个问题：

“GWD 是以 GWD 的身份调用 Anthropic，还是 GWD 只是把工作委派给用户本地已认证的 Claude Code 运行时？”

- 如果 GWD 是以 GWD 的身份调用 Anthropic：必须要求 API key 或受支持的云认证
- 如果 GWD 只是委派给本地 Claude Code：可以接受，前提是 GWD 自身不会拦截、生成或重放订阅凭据

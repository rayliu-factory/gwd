...
GWD 是一个 AI 编程代理，负责规划、执行、验证和交付，让你可以把注意力放在“要构建什么”上。本指南会带你完成 macOS、Windows 和 Linux 的安装，并启动你的第一个会话。
...
> **Apple Silicon PATH 修复：** 如果安装后找不到 `gwd`，可能是 npm 的全局 bin 目录没有加入 PATH：
> ```bash
> echo 'export PATH="$(npm prefix -g)/bin:$PATH"' >> ~/.zshrc
> source ~/.zshrc
> ```

> **oh-my-zsh 冲突：** oh-my-zsh 的 git 插件定义了 `alias gwd='git svn dcommit'`。可在 `~/.zshrc` 中加入 `unalias gwd 2>/dev/null`，或者改用 `gwd-cli`。
...
> - 如果 `gwd` 无法识别，先重启终端。Windows 需要新开终端才能读取更新后的 PATH。
...
如果你不想在宿主机安装 Node.js，可以在隔离沙箱中运行 GWD。
...
完成 provider 设置后，GWD 会自动选择一个默认模型。你可以在会话中随时切换：
...
在会话内输入 `/gwd`。GWD 会一次执行一个工作单元，并在每一步之间暂停，通过向导展示刚完成了什么、下一步是什么。

- **没有 `.gwd/` 目录**：启动讨论流程，先收集你的项目愿景
...
输入 `/gwd auto` 后就可以离开。GWD 会自主完成 research、planning、execution、verification、commit，并持续推进每个 slice，直到 milestone 完成。
...
/gwd discuss    # 讨论架构决策
/gwd status     # 查看进度
/gwd queue      # 排队下一个 milestone
```

两个终端都会读写同一套 `.gwd/` 文件。你在终端 2 里做出的决策，会在下一个阶段边界被自动拾取。
...
GWD 也提供 VS Code 扩展。你可以从扩展市场安装（publisher: FluxLabs），或者在 VS Code 扩展面板中直接搜索 “GWD”：

- **`@gwd` 聊天参与者**：在 VS Code Chat 中直接与 agent 对话
- **侧边栏仪表板**：显示连接状态、模型信息、Token 使用量
- **完整命令面板**：启动 / 停止 agent、切换模型、导出会话

GWD npm 包（`@appfiex-rayliu/gwd`）需要先安装好，扩展会通过 RPC 连接到 `gwd` CLI。

---

## Web 界面

GWD 也提供一个基于浏览器的可视化项目管理界面：
...
GWD 每 24 小时检查一次更新，并在启动时提示。你也可以手动更新：
...
| 问题 | 解决方式 |
|------|----------|
| `command not found: gwd` | 把 npm 全局 bin 目录加入 PATH（见上面的系统说明） |
| `gwd` 实际执行了 `git svn dcommit` | oh-my-zsh 冲突，执行 `unalias gwd` 或改用 `gwd-cli` |
| `npm install -g @appfiex-rayliu/gwd` 权限错误 | 修复 npm prefix（见 Linux 说明）或改用 nvm |
| 无法连接到 LLM | 用 `gwd config` 检查 API key，并确认网络可用 |
| `gwd` 启动时卡住 | 检查 Node.js 版本：`node --version`（需要 22+） |

...
Captures 允许你在自动模式执行过程中随手记录想法，而不必打断当前流程。你可以把新想法、bug 或范围变更记录下来，让 GWD 在 tasks 之间的自然间隙中进行分流处理。
...
这些 capture 会追加到 `.gwd/CAPTURES.md`，并在 tasks 之间自动参与 triage。
...
1. **Capture**：`/gwd capture "thought"` 会带着时间戳和唯一 ID 追加到 `.gwd/CAPTURES.md`
2. **Triage**：在 tasks 之间的自然衔接点（`handleAgentEnd` 中），GWD 会检测待处理 capture 并进行分类
...
Captures 总是写回**原始项目根目录**下的 `.gwd/CAPTURES.md`，而不是 worktree 的本地副本。这样从 steering 终端记录的内容，也能被运行在 worktree 里的自动模式会话看到。

## 命令

| 命令 | 说明 |
|------|------|
| `/gwd capture "text"` | 记录一个想法（单词时引号可省略） |

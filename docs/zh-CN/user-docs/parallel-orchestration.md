...
GWD 会扫描所有 milestones，检查依赖与文件重叠，给出一份可并行性报告，并为符合条件的 milestones 启动 workers。
...
每个 worker 都是一个完全隔离的独立 `gwd` 进程：

| 资源 | 隔离方式 |
|------|----------|
| **文件系统** | Git worktree：每个 worker 都有自己的 checkout |
| **Git 分支** | `milestone/<MID>`：每个 milestone 一条分支 |
| **状态推导** | 通过 `GWD_MILESTONE_LOCK` 环境变量，让 `deriveState()` 只看到被分配的 milestone |
| **上下文窗口** | 独立进程：每个 worker 都有自己的 agent sessions |
| **指标** | 每个 worktree 都有自己的 `.gwd/metrics.json` |
| **崩溃恢复** | 每个 worktree 都有自己的 `.gwd/auto.lock` |

### 协调方式

Workers 和 coordinator 通过基于文件的 IPC 通信：

- **会话状态文件**（`.gwd/parallel/<MID>.status.json`）：worker 写入 heartbeat，coordinator 读取
- **信号文件**（`.gwd/parallel/<MID>.signal.json`）：coordinator 写信号，worker 消费
- **原子写入**：使用写临时文件再 rename 的方式，避免读到半成品

## 可并行性分析

在真正启动并行执行之前，GWD 会先检查哪些 milestones 可以安全并发运行。
...
把下面内容加到 `~/.gwd/PREFERENCES.md` 或 `.gwd/PREFERENCES.md`：
...
| Key | 类型 | 默认值 | 说明 |
|-----|------|--------|------|
| `enabled` | boolean | `false` | 总开关。只有设为 `true`，`/gwd parallel` 命令才可用。 |
...
| `auto_merge` | `"auto"`、`"confirm"`、`"manual"` | `"confirm"` | merge-back 策略。`confirm` 会在合并前询问；`manual` 要求显式执行 `/gwd parallel merge`。 |

## 命令

| 命令 | 说明 |
|------|------|
| `/gwd parallel start` | 分析可并行性、确认并启动 workers |
| `/gwd parallel status` | 显示所有 workers 的状态、已完成单元和成本 |
| `/gwd parallel stop` | 停止所有 workers（发送 SIGTERM） |
| `/gwd parallel stop M002` | 停止某个指定 milestone 的 worker |
| `/gwd parallel pause` | 暂停所有 workers（完成当前单元后等待） |
| `/gwd parallel pause M002` | 暂停某个指定 worker |
| `/gwd parallel resume` | 恢复所有已暂停 workers |
| `/gwd parallel resume M002` | 恢复某个指定 worker |
| `/gwd parallel merge` | 把所有已完成 milestones 合并回 main |
| `/gwd parallel merge M002` | 只把某个指定 milestone 合并回 main |
...
1. `.gwd/` 状态文件（如 `STATE.md`、`metrics.json`）会**自动解决**，默认接受 milestone 分支版本
2. 代码冲突则会**停止并报告**。合并会暂停，并显示哪些文件冲突。你需要手动解决后，再执行 `/gwd parallel merge <MID>` 重试
...
`/gwd doctor` 能检测并行会话相关问题：

- **过期的并行会话**：worker 进程已经死亡，但没有清理干净。Doctor 会检查 `.gwd/parallel/*.status.json` 中记录的 PID 和 heartbeat，发现失效后自动清理。

可以执行 `/gwd doctor --fix` 自动清理。
...
| **`GWD_MILESTONE_LOCK`** | 每个 worker 在状态推导时只能看到自己的 milestone |
| **`GWD_PARALLEL_WORKER`** | Worker 不能再嵌套启动新的并行会话 |
| **预算上限** | 跨所有 workers 执行聚合成本限制 |
| **信号式关闭** | 通过文件信号 + SIGTERM 优雅停止 |
| **Doctor 集成** | 检测并清理孤儿会话 |
| **冲突感知 merge** | 遇到代码冲突时停止；`.gwd/` 状态冲突自动解决 |
...
`.gwd/parallel/` 和 `.gwd/worktrees/` 都会被 gitignore，因为它们只是运行时协调文件，永远不会提交。
...
说明所有 milestones 要么已完成，要么被依赖阻塞。可通过 `/gwd queue` 查看 milestone 状态和依赖链。
...
1. 执行 `/gwd doctor --fix` 清理 stale sessions
2. 执行 `/gwd parallel status` 查看当前状态
3. 重新执行 `/gwd parallel start`，为剩余 milestones 启动新的 workers

### 并行执行完成后发生 merge 冲突

1. 执行 `/gwd parallel merge` 查看哪些 milestones 存在冲突
2. 在 `.gwd/worktrees/<MID>/` 对应的 worktree 中手动解决冲突
3. 执行 `/gwd parallel merge <MID>` 重试

### Workers 看起来卡住了

先检查是否触达了预算上限：`/gwd parallel status` 会显示每个 worker 的成本。继续执行的话，提升 `parallel.budget_ceiling` 或直接移除它。

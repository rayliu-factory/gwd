...
这是一份覆盖 GWD 所有受支持 LLM providers 的分步配置指南。如果你已经运行过 onboarding 向导（`gwd config`）并选择了 provider，很可能已经配置完成，可以在会话中用 `/model` 检查。
...
内置 providers 的 models 已经预注册在 GWD 里。你只需要提供认证信息。
...
或者运行 `gwd config`，在提示时粘贴 key。

**获取 key：** [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys)

**选项 B：Claude Code CLI**

如果你有 Claude Pro 或 Max 订阅，可以通过 Anthropic 官方的 Claude Code CLI 完成认证。安装后执行 `claude` 登录，随后 GWD 会自动检测并经由该通道路由：
...
GWD 会检测你本地的 Claude Code 安装，并把它作为已认证的 Anthropic surface 使用。这是 Anthropic 订阅用户符合 TOS 的方式，GWD 不会直接处理你的订阅凭据。

> **注意：** GWD 不支持 Anthropic 的浏览器 OAuth 登录。请改用 API key 或 Claude Code CLI。

**选项 C：在 Claude Code 里直接用 Claude Pro / Max 订阅跑 GWD**

如果你已经有 Claude Pro / Max 订阅，并希望直接在 Claude Code 里使用 GWD 的 planning、execution 和 milestone orchestration，而不是切到单独终端，那么可以把 GWD 接成一个 MCP server。这样 Claude Code 就能通过 [Model Context Protocol](https://modelcontextprotocol.io) 使用 GWD 的完整 workflow 工具集，在你现有 Claude plan 的驱动下获得 GWD 的结构化项目管理能力。

**自动配置（推荐）**

当 GWD 在启动时检测到 Claude Code model，它会自动在项目根目录写入一个带有 GWD workflow MCP server 配置的 `.mcp.json` 文件。无需手动步骤，只要以 Claude Code 作为 provider 启动一次 GWD，配置就会自动生成。
...
这会在项目的 `.mcp.json` 中写入（或更新）`gwd-workflow` 条目。Claude Code 会在下一次启动会话时自动发现这个文件。

**手动配置**

如果你更希望自己配置，可以把 GWD 加到项目的 `.mcp.json` 中：
...
你也可以把这段配置写到 `~/.claude/settings.json` 的 `mcpServers` 中，让 GWD 在所有项目中都可用。

**暴露了什么**

MCP server 会暴露 GWD 的完整 workflow 工具面：milestone planning、task completion、slice 管理、roadmap reassessment、journal 查询等。会话管理工具（`gwd_execute`、`gwd_status`、`gwd_result`、`gwd_cancel`）允许 Claude Code 启动并监控 GWD 自动模式会话。完整工具列表见 [命令 → MCP Server 模式](./commands.md#mcp-server-mode)。

**验证连接**

在 GWD 会话里检查 MCP server 是否可达：
...
或者运行 `gwd config`，选择 “Paste an API key” 然后选择 “OpenAI”。
...
或者运行 `gwd config`，选择 “Paste an API key” 然后选择 “OpenRouter”。

**第 3 步：切换到 OpenRouter model**

在 GWD 会话中输入 `/model` 并选择一个 OpenRouter model。OpenRouter models 都以 `openrouter/` 为前缀（例如 `openrouter/anthropic/claude-sonnet-4`）。

**可选：通过 `models.json` 添加自定义 OpenRouter models**

如果你想使用不在内置列表中的 model，可把它写进 `~/.gwd/agent/models.json`：
...
注意：这里的 `apiKey` 字段写的是**环境变量名**，不是字面 key。GWD 会自动解析它。你也可以改用字面值或 shell 命令（见 [值解析](./custom-models.md#value-resolution)）。
...
本地 providers 运行在你的机器上。因为 GWD 需要知道 endpoint URL 和可用 models，所以它们都要求配置 `models.json`。

**配置文件位置：** `~/.gwd/agent/models.json`
...
**第 3 步：创建 `~/.gwd/agent/models.json`**
...
在 GWD 里输入 `/model`，然后选择你的 Ollama model。
...
任何实现了 OpenAI Chat Completions API 的 server 都可以和 GWD 配合使用。这包括代理（LiteLLM、Portkey、Helicone）、自托管推理服务，以及新出现的 providers。
...
这会自动帮你写好 `~/.gwd/agent/models.json`。
...
**原因：** key 虽然设在 shell 中，但 GWD 看不到。

**解决：** 确认你是在同一个终端里 `export` 了该环境变量并运行 `gwd`。或者直接用 `gwd config` 把 key 保存进 `~/.gwd/agent/auth.json`，这样就能跨会话持久化。

### OpenRouter models 没出现在 `/model`

**原因：** 没有设置 `OPENROUTER_API_KEY`，因此 GWD 会隐藏 OpenRouter models。
...
**解决：** 在 provider 配置里添加 `compat.supportsDeveloperRole: false`。这样 GWD 会改用 `system` message：
...
这是**预期行为**。GWD 对自定义 models 的默认成本就是 0。如果你想获得准确的成本跟踪，需要自己填写 `cost` 字段：
...
- `models.json` 是否是合法 JSON（可执行 `cat ~/.gwd/agent/models.json | python3 -m json.tool`）
- 本地 providers 的 server 是否已经运行

如果还需要更多帮助，请查看 [故障排查](./troubleshooting.md)，或者在会话中运行 `/gwd doctor`。

...
技能（Skills）是当当前 task 匹配时由 GWD 加载的专用指令集。它们为 LLM 提供领域化指导，例如编码模式、框架惯用法、测试策略和工具使用方式。

Skills 遵循开放的 [Agent Skills 标准](https://agentskills.io/)，并且**不是 GWD 专属格式**。它们同样适用于 Claude Code、OpenAI Codex、Cursor、GitHub Copilot、Windsurf 以及其他 40+ agent。

## 技能目录

GWD 会按优先级顺序从两个位置读取技能：
...
> **从 `~/.gwd/agent/skills/` 迁移：** 升级后首次启动时，GWD 会自动把旧版 `~/.gwd/agent/skills/` 中的技能复制到 `~/.agents/skills/`。旧目录会保留，以兼容旧流程。
...
在执行 `gwd init` 时，GWD 会检测项目技术栈并推荐合适的技能包。对于 brownfield 项目，检测是自动的；对于 greenfield 项目，则由用户选择技术栈。

这个精选目录维护在 `src/resources/extensions/gwd/skill-catalog.ts`。每一条目都会把一个技术栈映射到一个 skills.sh 仓库，以及其中的具体技能名称。
...
技能目录定义位于 [`src/resources/extensions/gwd/skill-catalog.ts`](../../../src/resources/extensions/gwd/skill-catalog.ts)。新增或更新一个技能包时：
...
`skill_discovery` 偏好控制 GWD 在自动模式中如何发现技能：
...
GWD 会跨自动模式会话跟踪技能表现，并提供健康度数据，帮助你持续维护技能质量。
...
通过 `/gwd skill-health` 查看技能表现：

```
/gwd skill-health              # 总览表：名称、使用次数、成功率、token、趋势、最近使用时间
/gwd skill-health rust-core    # 查看单个技能的详细信息
/gwd skill-health --stale 30   # 查看 30+ 天未使用的技能
/gwd skill-health --declining  # 查看成功率在下降的技能
```
...
如果把它配置为 post-unit hook，GWD 可以分析 agent 在执行中是否偏离了某个技能的指令。如果检测到明显漂移（例如 API 模式过时、指导错误），它会把建议修复写到 `.gwd/skill-review-queue.md`，供人工审核。

...
Captures let you fire-and-forget thoughts during auto-mode execution. Instead of pausing auto-mode to steer, you can capture ideas, bugs, or scope changes and let GWD triage them at natural seams between tasks.
...
1. **Capture** — `/gwd capture "thought"` appends to `.gwd/CAPTURES.md` with a timestamp and unique ID
2. **Triage** — at natural seams between tasks (in `handleAgentEnd`), GWD detects pending captures and classifies them
...
Captures always resolve to the **original project root's** `.gwd/CAPTURES.md`, not the worktree's local copy. This ensures captures from a steering terminal are visible to the auto-mode session running in a worktree.

## Commands

| Command | Description |
|---------|-------------|
| `/gwd capture "text"` | Capture a thought (quotes optional for single words) |

...
Skills are specialized instruction sets that GWD loads when the task matches. They provide domain-specific guidance for the LLM — coding patterns, framework idioms, testing strategies, and tool usage.

Skills follow the open [Agent Skills standard](https://agentskills.io/) and are **not GWD-specific** — they work with Claude Code, OpenAI Codex, Cursor, GitHub Copilot, Windsurf, and 40+ other agents.

## Skill Directories

GWD reads skills from two locations, in priority order:
...
> **Migration from `~/.gwd/agent/skills/`:** On first launch after upgrading, GWD automatically copies skills from the legacy `~/.gwd/agent/skills/` directory to `~/.agents/skills/`. The old directory is preserved for backward compatibility.
...
During `gwd init`, GWD detects the project's tech stack and recommends relevant skill packs. For brownfield projects, detection is automatic; for greenfield projects, the user picks a tech stack.

The curated catalog is maintained in `src/resources/extensions/gwd/skill-catalog.ts`. Each entry maps a tech stack to a skills.sh repo and specific skill names.
...
The skill catalog lives in [`src/resources/extensions/gwd/skill-catalog.ts`](../../src/resources/extensions/gwd/skill-catalog.ts). To add or update a pack:
...
GWD tracks skill performance across auto-mode sessions and surfaces health data to help you maintain skill quality.
...
View skill performance with `/gwd skill-health`:

```
/gwd skill-health              # overview table: name, uses, success%, tokens, trend, last used
...
When configured as a post-unit hook, GWD can analyze whether the agent deviated from a skill's instructions during execution. If significant drift is detected (outdated API patterns, incorrect guidance), it writes proposed fixes to `.gwd/skill-review-queue.md` for human review.

...
> **Phase 2 (Global Setup refactor)**: Deferred — `/gwd setup` provides routing to existing commands for now

## Problem Statement

GWD currently has two disconnected onboarding paths:

1. **App onboarding** (`onboarding.ts`) — pre-TUI, only handles LLM/tool auth. Runs once ever, no project awareness.
2. **Project bootstrap** (`showSmartEntry()`) — silently creates `.gwd/` and drops you into the discuss prompt with zero explanation.

Neither detects v1 `.planning/` directories, explains what GWD is, offers project-level configuration, or helps returning users entering a new folder.

## Design Decisions

### Q1: Should bare `/gwd` show a menu or auto-start?

**Answer: Contextual behavior based on detection.**

| Detected State | Behavior |
|---|---|
| No `.gwd/`, no `.planning/` | **Init Wizard** (new project onboarding) |
| No `.gwd/`, has `.planning/` | **Migration offer** → `/gwd migrate` or skip |
| Has `.gwd/`, no milestones | Current flow (discuss prompt) |
| Has `.gwd/`, has milestones | Current flow (smart entry / auto resume) |
| First-ever GWD launch (no `~/.gwd/`) | **Global setup** first, then project init |

### Q2: Should there be an onboarding wizard?

**Answer: Yes — two-tier wizard.**

- **Global wizard** (`/gwd setup`) — runs once per machine. Handles: LLM auth (absorb current `onboarding.ts`), global preferences (default model, mode, timeout defaults), tool keys, remote questions.
- **Project wizard** (`/gwd init`) — runs once per project folder. Handles: project-type detection, preferences template, git init, `.gitignore`, optional CONTEXT.md seeding.

Both wizards should be:
- Skippable at every step
- Re-runnable (`/gwd setup` and `/gwd init` work any time)
...
### New File: `src/resources/extensions/gwd/init-wizard.ts`

Project-level init wizard. Responsible for the interactive experience when entering a new folder.

### New File: `src/resources/extensions/gwd/detection.ts`

Pure detection functions. No UI, no side effects.

### Modified: `src/resources/extensions/gwd/guided-flow.ts`

`showSmartEntry()` gains a detection preamble before the current logic.

### Modified: `src/resources/extensions/gwd/commands.ts`

New subcommands: `/gwd init`, `/gwd setup`. Existing `/gwd migrate` stays.

### Modified: `src/onboarding.ts`

Refactored to be callable from both pre-TUI boot and `/gwd setup`.
...
interface ProjectDetection {
  ...
  /** Does ~/.gwd/ exist with preferences? */
  ...
  /** v2 details (if state === 'v2-gwd' or 'v2-gwd-empty') */
  v2?: {
...
Returns `true` if `~/.gwd/` doesn't exist or has no `PREFERENCES.md`.

---

## Phase 2: Global Setup Wizard (`/gwd setup`)
...
### Task 2.2: `/gwd setup` command handler

```
/gwd setup          → full wizard (all steps)
/gwd setup llm      → just LLM auth
/gwd setup search   → just web search
/gwd setup remote   → just remote questions
/gwd setup keys     → just tool keys
/gwd setup prefs    → just global preferences
```
...
│  ✓ Remote questions: Discord #gwd-bot        │
...
## Phase 3: Project Init Wizard (`/gwd init`)
...
| **`git.commit_docs`** | Whether `.gwd/` plans get committed to git. Team projects usually want `true`, throwaway prototypes want `false`. Affects the very first commit. | `true` |
...
#### Tier 3: Don't ask at init (defer to `/gwd prefs project`)
...
Step 1: Detection scan (automatic, instant, no prompt)
   ...
   ├─ .gwd/ already exists? → Re-init safety (Task 3.3)
...
Step 5: Verification Commands (auto-populated from detection)
   "How should GWD verify code changes?"
...
Step 6: Git Preferences
   "Git settings for this project:"
     Commit .gwd/ plans to git?  [Y/n]         ← default yes
...
Step 9: Bootstrap .gwd/ structure
   - Creates .gwd/milestones/
   - Creates .gwd/PREFERENCES.md (from wizard answers)
   - Creates .gitignore entries
   - Seeds CONTEXT.md with detected project signals
   - Commits "chore: init gwd" (if commit_docs enabled)
...
If `.gwd/` already exists when `/gwd init` is run:
...
// No .gwd/ — run project init wizard
if (detection.state === 'none') {
  await showProjectInit(ctx, pi, basePath, detection);
  return;
}

// Existing .gwd/ — current logic continues unchanged
```

### Task 4.2: Preserve zero-friction for returning users

The detection + init wizard only triggers when `.gwd/` doesn't exist.
Once `.gwd/` exists, the flow is identical to today — no regressions.
...
- `isFirstEverLaunch()` with/without `~/.gwd/`

### Task 5.2: Init wizard integration tests

- New folder → wizard triggers → `.gwd/` created correctly
- v1 folder → migration offer shown
- Existing `.gwd/` → wizard skipped, normal flow
- Re-run `/gwd init` on existing project → safe behavior

### Task 5.3: Global setup tests

- `/gwd setup` from command handler
- Individual sub-steps (`/gwd setup llm`, etc.)
...
| File | Action | Purpose |
|------|--------|---------|
| `src/resources/extensions/gwd/detection.ts` | **NEW** | Pure detection functions |
| `src/resources/extensions/gwd/init-wizard.ts` | **NEW** | Project init wizard UI |
| `src/resources/extensions/gwd/global-setup.ts` | **NEW** | Global setup wizard (refactored from onboarding.ts) |
| `src/onboarding.ts` | **MODIFY** | Delegate to global-setup.ts, keep boot integration |
| `src/resources/extensions/gwd/guided-flow.ts` | **MODIFY** | Add detection preamble to showSmartEntry() |
| `src/resources/extensions/gwd/commands.ts` | **MODIFY** | Add `/gwd init` and `/gwd setup` subcommands |
| Tests (TBD paths) | **NEW** | Detection, init, setup tests |

## Open Questions for Discussion

1. **Should `/gwd init` auto-transition to the discuss prompt?** Or end with "Run /gwd to start"? Auto-transition is more fluid but might feel jarring after a wizard.
...
3. **Should `/gwd setup` be accessible outside the TUI?** e.g. `gwd setup` from the shell before launching. Currently `onboarding.ts` handles this but it's limited.

4. **Migration: should we auto-detect `.planning/` in parent directories?** Some users might run GWD from a subdirectory while `.planning/` is at the repo root.

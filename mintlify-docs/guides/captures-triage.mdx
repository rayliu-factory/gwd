...
Captures let you fire-and-forget thoughts during auto-mode execution. Instead of pausing to steer, capture ideas, bugs, or scope changes and let GWD triage them at natural seams between tasks.
...
<Steps>
  <Step title="Capture">
    `/gwd capture "thought"` appends to `.gwd/CAPTURES.md` with a timestamp and unique ID.
  </Step>
  <Step title="Triage">
    At natural seams between tasks, GWD classifies each capture.
  </Step>
  <Step title="Confirm">
...
Captures resolve to the **original project root's** `.gwd/CAPTURES.md`, not the worktree's local copy. Captures from a steering terminal are visible to the auto-mode session running in a worktree.

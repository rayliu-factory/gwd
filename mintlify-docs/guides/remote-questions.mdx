...
Remote questions allow GWD to ask for user input via Slack, Discord, or Telegram when running in headless auto-mode. When GWD encounters a decision point, it posts the question to your configured channel and polls for a response.
...
| `/gwd remote status` | Show current config and last prompt status |

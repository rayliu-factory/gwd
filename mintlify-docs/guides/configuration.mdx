...
| `/gwd prefs global` | Global preferences wizard |
| `/gwd prefs project` | Project preferences wizard |
| `/gwd prefs status` | Show current files, merged values, and skill status |
...
To opt a project into staged project-level discovery:
...
Tool API keys are stored globally in `~/.gwd/agent/auth.json`. Set them once with `/gwd config`.
...
GWD connects to external MCP servers configured in project files:

- `.mcp.json` — repo-shared config
- `.gwd/mcp.json` — local-only config
...
Verify from a GWD session: `mcp_servers` → `mcp_discover` → `mcp_call`.

`secure_env_collect` will not set MCP server runtime variables such as `GWD_WORKFLOW_EXECUTORS_MODULE`, `GWD_WORKFLOW_PROJECT_ROOT`, `GWD_CLI_PATH`, `NODE_OPTIONS`, `NODE_PATH`, or `PATH`; configure those in the operator environment or MCP config. Local dotenv writes are hydrated into the current MCP server process, while Vercel and Convex pushes are remote-only and are not added to `process.env`.
...
When a model fails to switch, GWD automatically tries the next model in the `fallbacks` list.
...
Automatic parallel task dispatch inside a slice. It is enabled by default and only dispatches when task-plan IO annotations produce a non-ambiguous graph with enough ready, non-conflicting tasks.
...
|----------|---------|-------------|
| `GWD_HOME` | `~/.gwd` | Global GWD directory |
| `GWD_PROJECT_ID` | (auto-hash) | Override project identity hash |
| `GWD_STATE_DIR` | `$GWD_HOME` | Per-project state root |
| `GWD_CODING_AGENT_DIR` | `$GWD_HOME/agent` | Agent directory |

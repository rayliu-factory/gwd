...
`Ctrl+Alt+G` or `/gwd status` shows real-time cost breakdown by:
...
After two or more slices complete, GWD projects the remaining cost:
...
- Check `/gwd status` after a few slices to see per-slice averages

...
GWD's token optimization system has three pillars: **token profiles**, **context compression**, and **complexity-based task routing**.
...
GWD can apply deterministic text compression before falling back to section-boundary truncation:
...
GWD classifies each task by complexity and routes it to an appropriate model tier.
...
GWD tracks success/failure per tier and adjusts classifications over time. User feedback via `/gwd rate` is weighted 2x:

```
/gwd rate over    # model was overpowered
/gwd rate ok      # appropriate
/gwd rate under   # too weak
```

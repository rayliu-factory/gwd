---
title: "Auto mode"
description: "GWD's autonomous execution engine — run /gwd auto, walk away, come back to built software with clean git history."
---

Auto mode is a **state machine driven by the GWD database at the project root**. It derives the next unit of work from authoritative SQLite state, creates a fresh agent session with pre-loaded context, and lets the LLM execute. When the LLM finishes, auto mode persists the result to the database, refreshes `.gwd/` projections such as `STATE.md`, and dispatches the next unit.
...
Auto mode persists worker state, unit-dispatch state, and paused-session metadata in the project-root SQLite database. If the session dies, the next `/gwd auto` reconstructs the interrupted unit from DB-backed runtime state, synthesizes a recovery briefing from the conversation history written to disk, and resumes with full context. If the project-root database is unavailable, crash recovery degrades gracefully to a clean start instead of reconstructing interrupted work from disk.

**Headless auto-restart:** When running `gwd headless auto`, crashes trigger automatic restart with exponential backoff (5s → 10s → 30s cap, default 3 attempts). Combined with crash recovery, this enables overnight "run until done" execution.
...
A sliding-window analysis detects stuck loops — catching cycles like A→B→A→B as well as single-unit repeats. On detection, GWD retries once with a diagnostic prompt. If it fails again, auto mode stops with the exact file it expected.
...
GWD maintains a `KNOWLEDGE.md` file — an append-only register of project-specific rules, patterns, and lessons learned. The agent reads it at the start of every unit and appends when discovering recurring issues or non-obvious patterns.
...
After milestone completion, GWD auto-generates a self-contained HTML report with progress tree, dependency graph, cost/token metrics, execution timeline, and changelog.

```yaml
auto_report: true # enabled by default
```

Generate manually with `/gwd export --html`, or for all milestones with `/gwd export --html --all`.

### Reactive task execution

Reactive task execution is enabled by default. GWD derives a dependency graph from IO annotations in task plans and, when at least three ready tasks are available by default, dispatches non-conflicting tasks in parallel via subagents.
...
<Steps>
  <Step title="Start">``` /gwd auto ```</Step>
  <Step title="Pause">
    Press **Escape**. The conversation is preserved. You can interact with the
    agent, inspect state, or resume.
  </Step>
  <Step title="Resume">
    ``` /gwd auto ``` Auto mode reads DB-backed runtime state and picks up where
    it left off.
  </Step>
  <Step title="Stop">
    ``` /gwd stop ``` Stops auto mode gracefully. Can be run from a different
    terminal.
  </Step>
</Steps>
